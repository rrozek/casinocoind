...
static char casinocoinAlphabet[] =
...
{
    return encodeToken(type, token, size, casinocoinAlphabet);
}
...
/*  Base58 decode a Casinocoin token
...
static InverseAlphabet casinocoinInverse(casinocoinAlphabet);
...
{
    return decodeBase58Token(s, type, casinocoinInverse);
}
