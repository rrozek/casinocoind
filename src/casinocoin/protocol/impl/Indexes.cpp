...
{
    return getCasinocoinStateIndex (a, issue.account, issue.currency);
}
...
{
    return { ltCASINOCOIN_STATE,
        getCasinocoinStateIndex(id0, id1, currency) };
}
...
{
    return { ltCASINOCOIN_STATE,
        getCasinocoinStateIndex(id, issue) };
}
...
payChan (AccountID const& source, AccountID const& dst, std::uint32_t seq)
{
    sha512_half_hasher h;
    using beast::hash_append;
    hash_append(h, std::uint16_t(spaceCSCUChannel));
    hash_append(h, source);
    hash_append(h, dst);
    hash_append(h, seq);
