...
#ifndef CASINOCOIN_PROTOCOL_STPATHSET_H_INCLUDED
#define CASINOCOIN_PROTOCOL_STPATHSET_H_INCLUDED
...
public:
    ...
    STPathElement (
        ...
        , mIssuerID (issuer), is_offer_ (isCSC(mAccountID))
    {
        if (!is_offer_)
            mType |= typeAccount;

        if (forceCurrency || !isCSC(currency))
            mType |= typeCurrency;
