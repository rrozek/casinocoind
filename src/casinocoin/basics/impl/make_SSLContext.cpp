...
// the casinocoind server-server use case, where we only need MITM
...
get_context (std::string const& cipherList)
{
    ...
    // These are the raw DH parameters that Casinocoin Foundation has
    // chosen for Casinocoin, in the binary format needed by
    // d2i_DHparams.
    //
