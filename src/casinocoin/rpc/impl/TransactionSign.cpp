...
#include <casinocoin/app/tx/apply.h>              // Validity::Valid
...
#include <casinocoin/rpc/impl/LegacyPathFind.h>
    ...
    bool doPath)
{
    ...
    if (!tx_json.isMember (jss::Paths) && params.isMember (jss::build_path))
    {
...

        {
            ...
            if (ledger)
            {
                Pathfinder pf(std::make_shared<CasinocoinLineCache>(ledger),
    ...
    // Validate the fields in the serialized transaction.
    {
        ...
        if (!isLegalNet (fee))
        {
            ...
            err << "Invalid " << sfFee.fieldName
                << " field.  Fees must be specified in CSC.";
            return RPC::make_error (rpcINVALID_PARAMS, err.str ());
        }
        if (fee <= 0)
        {
