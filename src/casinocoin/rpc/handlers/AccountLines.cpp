...
struct VisitData
{
    std::vector <CasinocoinState::pointer> items;
    AccountID const& accountID;
    bool hasPeer;
    AccountID const& raPeerAccount;
};

void addLine (Json::Value& jsonLines, CasinocoinState const& line)
{
    ...
    if (line.getNoCasinocoin ())
        jPeer[jss::no_casinocoin] = true;
    if (line.getNoCasinocoinPeer ())
        jPeer[jss::no_casinocoin_peer] = true;
    if (line.getFreeze ())
...
Json::Value doAccountLines (RPC::Context& context)
{
    ...
    if (params.isMember (jss::marker))
    {
        ...
        auto const sleLine = ledger->read({ltCASINOCOIN_STATE, startAfter});
        ...
        auto const line = CasinocoinState::makeItem (accountID, sleLine);
        if (line == nullptr)
            return rpcError (rpcINVALID_PARAMS);
...

    {
        if (! forEachItemAfter(*ledger, accountID,
            ...
            {
                auto const line =
                    CasinocoinState::makeItem (visitData.accountID, sleCur);
                if (line != nullptr &&
                    (! visitData.hasPeer ||
    ...
    if (visitData.items.size () == reserve)
    {
        ...
        CasinocoinState::pointer line (visitData.items.back ());
