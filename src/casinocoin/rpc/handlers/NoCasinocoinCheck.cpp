...
// }
Json::Value doNoCasinocoinCheck (RPC::Context& context)
{
    ...
    unsigned int limit;
    if (auto err = readLimitField(limit, RPC::Tuning::noCasinocoinCheck, context))
        return *err;
    ...
    bool bDefaultCasinocoin = sle->getFieldU32 (sfFlags) & lsfDefaultCasinocoin;

    if (bDefaultCasinocoin & ! roleGateway)
    {
        problems.append ("You appear to have set your default casinocoin flag even though you "
            "are not a gateway. This is not recommended unless you are experimenting");
    }
    else if (roleGateway & ! bDefaultCasinocoin)
    {
        problems.append ("You should immediately set your default casinocoin flag");
        if (transactions)
        {
    ...
    forEachItemAfter (*ledger, accountID,
        ...
        {
            if (ownedItem->getType() == ltCASINOCOIN_STATE)
            {
                bool const bLow = accountID == ownedItem->getFieldAmount(sfLowLimit).getIssuer();

                bool const bNoCasinocoin = ownedItem->getFieldU32(sfFlags) &
                    (bLow ? lsfLowNoCasinocoin : lsfHighNoCasinocoin);

                std::string problem;
                bool needFix = false;
                if (bNoCasinocoin & roleGateway)
                {
                    problem = "You should clear the no casinocoin flag on your ";
                    needFix = true;
                }
                else if (! roleGateway & ! bNoCasinocoin)
                {
                    problem = "You should probably set the no casinocoin flag on your ";
                    needFix = true;
                }
                if (needFix)
                {
