...
Json::Value doBookOffers (RPC::Context& context)
{
    ...
    {
        pay_issuer = cscAccount ();
    }

    if (isCSC (pay_currency) && ! isCSC (pay_issuer))
        return RPC::make_error (
            rpcSRC_ISR_MALFORMED, "Unneeded field 'taker_pays.issuer' for "
            "CSC currency specification.");
    ...
    {
        get_issuer = cscAccount ();
    }


    if (isCSC (get_currency) && ! isCSC (get_issuer))
        return RPC::make_error (rpcDST_ISR_MALFORMED,
    ...
    if (!isCSC (get_currency) && isCSC (get_issuer))
        return RPC::make_error (rpcDST_ISR_MALFORMED,
            "Invalid field 'taker_gets.issuer', expected non-CSC issuer.");
