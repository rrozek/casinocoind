...
#ifndef CASINOCOIN_APP_CONSENSUS_CCLCXTX_H_INCLUDED
#define CASINOCOIN_APP_CONSENSUS_CCLCXTX_H_INCLUDED
...
/** Represents a transaction in CCLConsensus.

    CCLCxTx is a thin wrapper over the SHAMapItem that corresponds to the
    transaction.
*/
class CCLCxTx
{
public:
    ...
    */
    CCLCxTx(SHAMapItem const& txn) : tx_{txn}
    {
    }
...
/** Represents a set of transactions in CCLConsensus.

    CCLTxSet is a thin wrapper over a SHAMap that stores the set of
    transactions.
*/
class CCLTxSet
{
public:
    ...
    class MutableTxSet
    {
        friend class CCLTxSet;
        //! The SHAMap representing the transactions.
        std::shared_ptr<SHAMap> map_;

    public:
        MutableTxSet(CCLTxSet const& src) : map_{src.map_->snapShot(true)}
        {
        }
    ...
    */
    CCLTxSet(std::shared_ptr<SHAMap> m) : map_{std::move(m)}
    {
        assert(map_);
    }
     ...
     */
    CCLTxSet(MutableTxSet const& m) : map_{m.map_->snapShot(false)}
    {
    }
    ...
    */
    std::map<Tx::ID, bool>
    compare(CCLTxSet const& j) const
    {
        SHAMap::Delta delta;
