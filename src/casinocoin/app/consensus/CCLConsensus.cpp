...
#include <casinocoin/app/ledger/BuildLedger.h>
...
CCLConsensus::Adaptor::acquireLedger(LedgerHash const& hash)
{
...
void
CCLConsensus::Adaptor::propose(CCLCxPeerPos::Proposal const& proposal)
{
...
void
CCLConsensus::Adaptor::share(CCLTxSet const& txns)
{
    inboundTransactions_.giveSet(txns.id(), txns.map_, false);
}

boost::optional<CCLTxSet>
CCLConsensus::Adaptor::acquireTxSet(CCLTxSet::ID const& setId)
{
    if (auto txns = inboundTransactions_.getSet(setId, true))
    {
        return CCLTxSet{std::move(txns)};
    }
    return boost::none;
}

bool
CCLConsensus::Adaptor::hasOpenTransactions() const
{
    return !app_.openLedger().empty();
}

std::size_t
CCLConsensus::Adaptor::proposersValidated(LedgerHash const& h) const
{
    return app_.getValidations().numTrustedForLedger(h);
}

std::size_t
CCLConsensus::Adaptor::proposersFinished(
    CCLCxLedger const& ledger,
    LedgerHash const& h) const
{
    CCLValidations& vals = app_.getValidations();
    return vals.getNodesAfter(
        CCLValidatedLedger(ledger.ledger_, vals.adaptor().journal()), h);
}

uint256
CCLConsensus::Adaptor::getPrevLedger(
    uint256 ledgerID,
    CCLCxLedger const& ledger,
    ConsensusMode mode)
{
    ...
    uint256 netLgr = vals.getPreferred(
        CCLValidatedLedger{ledger.ledger_, vals.adaptor().journal()},
    ...
    ConsensusMode mode) -> Result
{
    ...
    if (!wrongLCL)
    {
        ...
        CCLCensorshipDetector<TxID, LedgerIndex>::TxIDSeqVec proposed;
...
void
CCLConsensus::Adaptor::onForceAccept(
    Result const& result,
    ...
    std::set<TxID>& failedTxs)
{
    ...
    return CCLCxLedger{std::move(built)};
}

void
CCLConsensus::Adaptor::validate(CCLCxLedger const& ledger,
    CCLTxSet const& txns,
    bool proposing)
{
    using namespace std::chrono_literals;
...
void
CCLConsensus::Adaptor::onModeChange(
...
bool
CCLConsensus::peerProposal(
    NetClock::time_point const& now,
    CCLCxPeerPos const& newProposal)
{
    ScopedLockType _{mutex_};
...
bool
CCLConsensus::Adaptor::preStartRound(CCLCxLedger const & prevLgr)
{
