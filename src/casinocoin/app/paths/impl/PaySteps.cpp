...
bool checkNear (CSCAmount const& expected, CSCAmount const& actual)
{
    return expected == actual;
};

static
bool isCSCAccount (STPathElement const& pe)
{
    if (pe.getNodeType () != STPathElement::typeAccount)
        return false;
    return isCSC (pe.getAccountID ());
};
    ...
    Issue const& curIssue)
{
    ...
    if (ctx.isFirst && e1->isAccount () &&
        ...
        isCSC (e1->getCurrency ()))
    {
        return make_CSCEndpointStep (ctx, e1->getAccountID ());
    }

    if (ctx.isLast && isCSCAccount (*e1) && e2->isAccount())
        return make_CSCEndpointStep (ctx, e2->getAccountID());
    ...
    if (isCSC (curIssue.currency) && isCSC (outCurrency))
    {
        JLOG (j.warn()) << "Found csc/csc offer payment step";
        return {temBAD_PATH, std::unique_ptr<Step>{}};
    }

    assert (e2->isOffer ());

    if (isCSC (outCurrency))
        return make_BookStepIX (ctx, curIssue);

    if (isCSC (curIssue.currency))
    ...
    beast::Journal j)
{
    if (isCSC (src))
    {
        JLOG (j.debug()) << "toStrand with cscAccount as src";
        return {temBAD_PATH, Strand{}};
    }
    if (isCSC (dst))
    {
        JLOG (j.debug()) << "toStrand with cscAccount as dst";
        return {temBAD_PATH, Strand{}};
    }
    ...
    beast::Journal j)
{
    ...
    for (auto const& pe : path)
    {
        ...
        if (hasIssuer && isCSC(pe.getIssuerID()))
            return {temBAD_PATH, Strand{}};

        if (hasAccount && isCSC(pe.getAccountID()))
            return {temBAD_PATH, Strand{}};

        if (hasCurrency && hasIssuer &&
            isCSC(pe.getCurrency()) != isCSC(pe.getIssuerID()))
            return {temBAD_PATH, Strand{}};
    ...
    */
        ...
        */
        ...
        if (cur->isAccount() && next->isAccount())
        {
                ...
                curIssue.account != next->getAccountID ())
            {
                ...
                result.push_back (std::move (msr.second));
                impliedPE.emplace(STPathElement::typeAccount,
                    curIssue.account, cscCurrency(), cscAccount());
                cur = &*impliedPE;
            }
        }
        else if (cur->isAccount() && next->isOffer())
        ...
        else if (cur->isOffer() && next->isAccount())
        {
                ...
                !isCSC (next->getAccountID ()))
            {
                if (isCSC(curIssue))
                {
                    ...
                    {
                        // Last step. insert csc endpoint step
                        auto msr = make_CSCEndpointStep (ctx(), next->getAccountID());
                        if (msr.first != tesSUCCESS)
                            return {msr.first, Strand{}};
                        result.push_back(std::move(msr.second));
                    }
    ...
    auto checkStrand = [&]() -> bool {
        auto stepAccts = [](Step const& s) -> std::pair<AccountID, AccountID> {
            ...
            return std::make_pair(cscAccount(), cscAccount());
        };
    ...
    beast::Journal j)
{
    ...
    {
        JLOG (j.debug())
            << "Flow: Invalid transaction: No paths and direct casinocoin not allowed.";
        return {temCASINOCOIN_EMPTY, std::vector<Strand>{}};
    }
...
bool
isDirectCscToCsc(Strand const& strand)
{
    return false;
}
...
template<>
bool
isDirectCscToCsc<CSCAmount, CSCAmount> (Strand const& strand)
{
...
template
bool
isDirectCscToCsc<CSCAmount, IOUAmount> (Strand const& strand);
template
bool
isDirectCscToCsc<IOUAmount, CSCAmount> (Strand const& strand);
template
bool
isDirectCscToCsc<IOUAmount, IOUAmount> (Strand const& strand);
