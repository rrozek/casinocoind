...
public:
        ...
        bool prevStepRedeems) const
    {
        ...
        auto rate = [&](AccountID const& id) {
            if (isCSC(id) || id == this->strandDst_)
                return parityRate;
...
template <class TIn, class TOut, class TDerived>
static
bool equalHelper (Step const& step, casinocoin::Book const& book)
{
...
bool bookStepEqual (Step const& step, casinocoin::Book const& book)
{
    bool const inCSC = isCSC (book.in.currency);
    bool const outCSC = isCSC (book.out.currency);
    if (inCSC && outCSC)
        return equalHelper<CSCAmount, CSCAmount,
            BookPaymentStep<CSCAmount, CSCAmount>> (step, book);
    if (inCSC && !outCSC)
        return equalHelper<CSCAmount, IOUAmount,
            BookPaymentStep<CSCAmount, IOUAmount>> (step, book);
    if (!inCSC && outCSC)
        return equalHelper<IOUAmount, CSCAmount,
            BookPaymentStep<IOUAmount, CSCAmount>> (step, book);
    if (!inCSC && !outCSC)
...
make_BookStepIX (
    ...
    Issue const& in)
{
    return make_BookStepHelper<IOUAmount, CSCAmount> (ctx, in, cscIssue());
}
...
make_BookStepXI (
    ...
    Issue const& out)
{
    return make_BookStepHelper<CSCAmount, IOUAmount> (ctx, cscIssue(), out);
}
