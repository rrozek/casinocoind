...
#ifndef CASINOCOIN_PATH_IMPL_AMOUNTSPEC_H_INCLUDED
#define CASINOCOIN_PATH_IMPL_AMOUNTSPEC_H_INCLUDED
...
#endif
    explicit
    EitherAmount (CSCAmount const& a)
            :csc(a)
    {
#ifndef NDEBUG
        native = true;
...
inline
CSCAmount&
get<CSCAmount> (EitherAmount& amt)
{
    assert (amt.native);
    return amt.csc;
}
...
inline
CSCAmount const&
get<CSCAmount> (EitherAmount const& amt)
{
    assert (amt.native);
    return amt.csc;
}
...
toAmountSpec (STAmount const& amt)
{
    ...
    result.native = isCSC (amt);
    if (result.native)
    {
        result.csc = CSCAmount (sMant);
    }
    else
    {
