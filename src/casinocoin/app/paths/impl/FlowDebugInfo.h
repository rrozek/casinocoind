...
#ifndef CASINOCOIN_PATH_IMPL_FLOWDEBUGINFO_H_INCLUDED
#define CASINOCOIN_PATH_IMPL_FLOWDEBUGINFO_H_INCLUDED
...
struct FlowDebugInfo
{
    ...
    to_string (bool writePassInfo) const
    {
        ...
        if (writePassInfo)
        {
            ...
            auto writeNestedCscAmtList = [&ostr, &writeCscAmtList](
                std::vector<std::vector<EitherAmount>> const& amts) {
                ostr << '[';
                if (!amts.empty ())
                {
                    writeCscAmtList(amts[0], '|');
                    for (size_t i = 1, e = amts.size (); i < e; ++i)
                    {
                        ostr << ';';
                        writeCscAmtList(amts[i], '|');
                    }
                }
                ostr << ']';
            };
            ...
            if (passInfo.nativeIn)
                writeCscAmtList (passInfo.in);
            else
                writeIouAmtList (passInfo.in);
            ostr << ", out_pass: ";
            if (passInfo.nativeOut)
                writeCscAmtList (passInfo.out);
            else
                ...
                !passInfo.liquiditySrcIn.back ().empty ())
            {
                ostr << ", l_src_in: ";
                if (passInfo.nativeIn)
                    writeNestedCscAmtList (passInfo.liquiditySrcIn);
                else
                    writeNestedIouAmtList (passInfo.liquiditySrcIn);
                ostr << ", l_src_out: ";
                if (passInfo.nativeOut)
                    writeNestedCscAmtList (passInfo.liquiditySrcOut);
                else
...
{
    return {sb.balanceChanges (rv), sb.cscDestroyed ()};
}
...
balanceDiffsToString (boost::optional<BalanceDiffs> const& bd)
{
    ...
    auto const& cscDestroyed = bd->second;
    std::ostringstream ostr;
    ostr << ", cscDestroyed: " << to_string (cscDestroyed);
    ostr << ", balanceDiffs: ";
