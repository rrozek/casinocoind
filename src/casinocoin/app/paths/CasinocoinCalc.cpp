...
CasinocoinCalc::Output CasinocoinCalc::casinocoinCalculate (
    PaymentSandbox& view,
    ...
    // Issuer:
    //      CSC: cscAccount()
    //  non-CSC: uSrcAccountID (for any issuer) or another account with
    //           trust node.
    STAmount const& saMaxAmountReq,             // --> -1 = no limit.

    // Issuer:
    //      CSC: cscAccount()
    //  non-CSC: uDstAccountID (for any issuer) or another account with
    //           trust node.
    ...
    Input const* const pInputs)
{
    ...
    if (callFlowV1)
    {
        ...
        auto result = rc.casinocoinCalculate (compareFlowV1V2 ? &flowV1FlowDebugInfo : nullptr);
...
bool CasinocoinCalc::addPathState(STPath const& path, TER& resultCode)
{
    ...
    if (pathState->status() == tesSUCCESS)
        pathState->checkNoCasinocoin (uDstAccountID_, uSrcAccountID_);
...
TER CasinocoinCalc::casinocoinCalculate (detail::FlowDebugInfo* flowDebugInfo)
{
    JLOG (j_.trace())
        << "casinocoinCalc>"
    ...
    {
        JLOG (j_.debug())
            << "casinocoinCalc: Invalid transaction:"
            << "No paths and direct casinocoin not allowed.";

        return temCASINOCOIN_EMPTY;
    }

    // Build a default path.  Use saDstAmountReq_ and saMaxAmountReq_ to imply
    // nodes.
    // XXX Might also make a CSC bridge by default.

    JLOG (j_.trace())
        << "casinocoinCalc: Paths in set: " << spsPaths_.size ();
        ...
        for (auto pathState : pathStateList_)
        {
                ...
                // Only do active paths.
            {
                ...
                else if (pathState->outPass() == zero)
                {
                    ...
                    JLOG (j_.warn())
                        << "casinocoinCalc: Non-dry path moves no funds";
