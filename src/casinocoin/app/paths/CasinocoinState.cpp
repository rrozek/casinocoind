...
CasinocoinState::pointer
CasinocoinState::makeItem (
    AccountID const& accountID,
        std::shared_ptr<SLE const> sle)
{
    // VFALCO Does this ever happen in practice?
    if (! sle || sle->getType () != ltCASINOCOIN_STATE)
        return {};
    return std::make_shared<CasinocoinState>(
        std::move(sle), accountID);
}

CasinocoinState::CasinocoinState (
    std::shared_ptr<SLE const>&& sle,
...
Json::Value CasinocoinState::getJson (int)
{
    ...
    ReadView const& view)
{
    ...
    forEachItem(view, accountID,
        ...
        {
             auto ret = CasinocoinState::makeItem (accountID, sleCur);
             if (ret)
                items.push_back (ret);
        });
