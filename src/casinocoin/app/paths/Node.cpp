...
Json::Value Node::getJson () const
{
    ...
    bool const hasCurrency = !isCSC (issue_.currency);
    bool const hasAccount = !isCSC (account_);
    bool const hasIssuer = !isCSC (issue_.account);
    ...
    if (!isCSC (issue_.currency))
        jvNode[jss::currency] = to_string (issue_.currency);

    if (!isCSC (issue_.account))
