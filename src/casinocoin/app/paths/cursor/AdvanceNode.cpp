...
TER PathCursor::advanceNode (STAmount const& amount, bool reverse, bool callerHasLiquidity) const
{
    ...
    PathCursor withMultiQuality {casinocoinCalc_, pathState_, multi, j_, nodeIndex_};
        ...
        else
        {
            ...
            else
            {
                            ...
                            view().parentCloseTime().time_since_epoch().count()))
                {
                    ...
                    casinocoinCalc_.permanentlyUnfundedOffers_.insert(
                        node().offerIndex_);
                    continue;
                }
                ...
                if (node().saTakerPays <= zero || node().saTakerGets <= zero)
                {
                    ...
                    else if (casinocoinCalc_.permanentlyUnfundedOffers_.find (index)
                             != casinocoinCalc_.permanentlyUnfundedOffers_.end ())
                    {
                ...
                auto itPast = casinocoinCalc_.mumSource_.find (accountIssue);
                bool bFoundPast = (itPast != casinocoinCalc_.mumSource_.end ());
                ...
                if (node().saOfferFunds <= zero)
                {
                    ...
                    if (bReverse && !bFoundReverse && !bFoundPast)
                    {
                        ...
                        // still do deletions.
                        // Mark offer for always deletion.
                        casinocoinCalc_.permanentlyUnfundedOffers_.insert (node().offerIndex_);
                    }
                    else
                    {
