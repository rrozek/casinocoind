...
// - If next node is an offer and output is non-CSC then we are the issuer and
//   do not need to push funds.
// - If next node is an offer and output is CSC then we need to deliver funds to
//   limbo.
TER PathCursor::forwardLiquidityForAccount () const
{
    ...
    if (previousNode().isAccount() && nextNode().isAccount())
    {
        ...
        if (!nodeIndex_)
        {
            ...
            // For the first node, calculate amount to casinocoin based on what is
            // available.
        ...
        else if (nodeIndex_ == lastNodeIndex)
        {
            ...
            {
                // Actually receive.
                resultCode = casinocoinCredit(view(),
                    previousAccountID,
                    node().account_,
    ...
    else if (previousNode().isAccount() && !nextNode().isAccount())
    {
        ...
        if (nodeIndex_)
        {
            // Non-CSC, current node is the issuer.
            JLOG (j_.trace())
                << "forwardLiquidityForAccount: account --> "
                << "ACCOUNT --> offer";
        ...
        else
        {
            ...
            if (pathState_.inReq() >= zero)
            {
                ...
                // Limit CSC by available. No limit for non-CSC as issuer.
                if (isCSC (node().issue_))
                    node().saFwdDeliver = std::min (
                        node().saFwdDeliver,
                        accountHolds(view(),
                            node().account_,
                            cscCurrency(),
                            cscAccount(),
                            fhIGNORE_FREEZE, viewJ)); // CSC can't be frozen
            ...
            else if (!isCSC (node().issue_))
            {
                // Non-CSC, current node is the issuer.
                // We could be delivering to multiple accounts, so we don't know
                // which casinocoin balance will be adjusted.  Assume just issuing.
            ...
            else
            {
                ...
                // Deliver CSC to limbo.
                resultCode = accountSend(view(),
                    node().account_, cscAccount(), node().saFwdDeliver, viewJ);
            }
        }
    }
    else if (!previousNode().isAccount() && nextNode().isAccount())
