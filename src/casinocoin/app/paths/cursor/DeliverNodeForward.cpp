    ...
    bool callerHasLiquidity) const
{
    ...
    while (resultCode == tesSUCCESS && saInAct + saInFees < saInReq)
    {
        ...
        else if (resultCode == tesSUCCESS)
        {
            ...
            if (!isCSC(nextNode().account_))
            {
                // ? --> OFFER --> account
                // Input fees: vary based upon the consumed offer's owner.
                // Output fees: none as CSC or the destination account is the
                // issuer.
                ...
                // Output: Debit offer owner, send CSC or non-XPR to next
                // account.
            ...
            else
            {
                ...
                // fees).
                auto const& id = isCSC(node().issue_) ?
                        cscAccount() : node().issue_.account;
            ...
            // is redundant.
            if (isCSC (previousNode().issue_.currency)
                || uInAccountID != node().offerOwnerAccount_)
            {
                auto id = !isCSC(previousNode().issue_.currency) ?
                        uInAccountID : cscAccount();
                resultCode = accountSend(view(),
                    id,
                    node().offerOwnerAccount_,
                    saInPassAct,
