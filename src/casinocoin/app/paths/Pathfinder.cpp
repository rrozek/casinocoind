...
The pathfinding request is identified by category, CSC to CSC, CSC to
non-CSC, non-CSC to CSC, same currency non-CSC to non-CSC, cross-currency
non-CSC to non-CSC.  For each category, there is a table of paths that the
...
Pathfinder::Pathfinder (
    std::shared_ptr<CasinocoinLineCache> const& cache,
    ...
    :   mSrcAccount (uSrcAccount),
        ...
        mEffectiveDst (isCSC(saDstAmount.getIssuer ()) ?
        ...
        mSrcAmount (srcAmount.value_or(STAmount({uSrcCurrency,
            uSrcIssuer.value_or(isCSC(uSrcCurrency) ?
                cscAccount() : uSrcAccount)}, 1u, 0, true))),
...
{
    assert (! uSrcIssuer || isCSC(uSrcCurrency) == isCSC(uSrcIssuer.get()));
}
...
bool Pathfinder::findPaths (int searchLevel)
{
    ...
    auto currencyIsCSC = isCSC (mSrcCurrency);

    bool useIssuerAccount
            = mSrcIssuer && !currencyIsCSC && !isCSC (*mSrcIssuer);
    auto& account = useIssuerAccount ? *mSrcIssuer : mSrcAccount;
    auto issuer = currencyIsCSC ? AccountID() : account;
    ...
    bool bSrcCsc = isCSC (mSrcCurrency);
    bool bDstCsc = isCSC (mDstAmount.getCurrency());
    ...
    if (! mLedger->exists (keylet::account (mDstAccount)))
    {
        ...
        {
            JLOG (j_.debug())
                    << "New account not being funded in CSC ";
            return false;
        }
    ...
    {
        // CSC -> CSC
        JLOG (j_.debug()) << "CSC to CSC payment";
        paymentType = pt_CSC_to_CSC;
    }
    else if (bSrcCsc)
    {
        // CSC -> non-CSC
        JLOG (j_.debug()) << "CSC to non-CSC payment";
        paymentType = pt_CSC_to_nonCSC;
    }
    else if (bDstCsc)
    {
        // non-CSC -> CSC
        JLOG (j_.debug()) << "non-CSC to CSC payment";
        paymentType = pt_nonCSC_to_CSC;
    }
    else if (mSrcCurrency == mDstAmount.getCurrency ())
    {
        // non-CSC -> non-CSC - Same currency
        JLOG (j_.debug()) << "non-CSC to non-CSC - same currency";
        paymentType = pt_nonCSC_to_same;
    }
    else
    {
        // non-CSC to non-CSC - Different currency
        JLOG (j_.debug()) << "non-CSC to non-CSC - cross currency";
        paymentType = pt_nonCSC_to_nonCSC;
    }
    ...
    uint64_t& qualityOut) const    // OUT: The returned initial quality
{
    ...
    path::CasinocoinCalc::Input rcInput;
    ...
    try
    {
        ...
        auto rc = path::CasinocoinCalc::casinocoinCalculate (
            sandbox,
            mSrcAmount,
            minDstAmount,
            mDstAccount,
        ...
        if (! convert_all_)
        {
            ...
            rcInput.partialPaymentAllowed = true;
            rc = path::CasinocoinCalc::casinocoinCalculate (
                sandbox,
                mSrcAmount,
    ...
    AccountID const& srcIssuer)
{
    ...
    const bool issuerIsSender = isCSC (mSrcCurrency) || (srcIssuer == mSrcAccount);
    ...
    AccountID const& dstAccount)
{
    ...
    if (!bFrozen)
    {
        ...
        for (auto const& item : mRLCache->getCasinocoinLines (account))
        {
            CasinocoinState* rspEntry = (CasinocoinState*) item.get ();
            ...
            else if (rspEntry->getNoCasinocoinPeer ())
            {
...
STPathSet& Pathfinder::addPathsForType (PathType const& pathType)
{
    ...
    case nt_CSC_BOOK:
        addLinks (parentPaths, pathsOut, afADD_BOOKS | afOB_CSC);
        break;
...
bool Pathfinder::isNoCasinocoin (
    AccountID const& fromAccount,
    AccountID const& toAccount,
    Currency const& currency)
{
    auto sleCasinocoin = mLedger->read(keylet::line(
        toAccount, fromAccount, currency));

    auto const flag ((toAccount > fromAccount)
                     ? lsfHighNoCasinocoin : lsfLowNoCasinocoin);

    return sleCasinocoin && (sleCasinocoin->getFieldU32 (sfFlags) & flag);
}

// Does this path end on an account-to-account link whose last account has
// set "no casinocoin" on the link?
bool Pathfinder::isNoCasinocoinOut (STPath const& currentPath)
{
    ...
    // no casinocoin on the output. A path with no casinocoin on its output can't be
    // followed by a link with no casinocoin on its input.
    auto const& fromAccount = (currentPath.size () == 1)
        ? mSrcAccount
        : (currentPath.end () - 2)->getAccountID ();
    auto const& toAccount = endElement.getAccountID ();
    return isNoCasinocoin (fromAccount, toAccount, endElement.getCurrency ());
}
    ...
    int addFlags)
{
    ...
    bool const bOnCSC = uEndCurrency.isZero ();
    ...
    if (addFlags & afADD_ACCOUNTS)
    {
        ...
        if (bOnCSC)
        {
            ...
            { // non-default path to CSC destination
                JLOG (j_.trace())
        ...
        else
        {
            ...
            if (sleEnd)
            {
                ...
                bool const bIsNoCasinocoinOut (
                    isNoCasinocoinOut (currentPath));
                bool const bDestOnly (
                    addFlags & afAC_LAST);

                auto& casinocoinLines (mRLCache->getCasinocoinLines (uEndAccount));

                AccountCandidates candidates;
                candidates.reserve (casinocoinLines.size ());

                for (auto const& item : casinocoinLines)
                {
                    auto* rs = dynamic_cast<CasinocoinState const *> (item.get ());
                    if (!rs)
                    {
                        JLOG (j_.error())
                                << "Couldn't decipher CasinocoinState";
                        continue;
                    }
                        ...
                        !currentPath.hasSeen (acct, uEndCurrency, acct))
                    {
                        ...
                        else if (bIsNoCasinocoinOut && rs->getNoCasinocoin ())
                        {
    ...
    {
        // add order books
        if (addFlags & afOB_CSC)
        {
            // to CSC only
            if (!bOnCSC && app_.getOrderBookDB ().isBookToCSC (
            ...
            for (auto const& book : books)
            {
                     ...
                     (book->getCurrencyOut () == mDstAmount.getCurrency ())))
                {
                    ...
                    { // to CSC
                        ...
                        {
                            // destination is CSC, add account and path is
                            // complete
                            JLOG (j_.trace())
                        ...
                        book->getIssuerOut ()))
                    {
                            ...
                            (newPath[newPath.size() - 2].isOffer ()))
                        {
                            ...
                            newPath[newPath.size() - 1] = STPathElement (
                                ...
                                cscAccount(), book->getCurrencyOut(),
                                book->getIssuerOut());
                        }
                        else
                        {
...
Pathfinder::PathType makePath (char const *string)
{
    ...
    while (true)
    {
        switch (*string++)
        {
            ...
            case 'x': // csc book
                ret.push_back (Pathfinder::nt_CSC_BOOK);
                break;
...
void Pathfinder::initPathTable ()
{
    ...
    fillPaths(
        pt_nonCSC_to_CSC, {
            {1, "sxd"},       // gateway buys CSC
            {2, "saxd"},      // source -> gateway -> book(CSC) -> dest
            {6, "saaxd"},
            {7, "sbxd"},
            {8, "sabxd"},
            {9, "sabaxd"}
    ...
    // non-CSC to non-CSC (same currency)
    fillPaths(
        pt_nonCSC_to_same,  {
            ...
            {6, "sxfad"},
            {6, "safad"},
            {6, "saxfd"},   // source -> gateway -> book to CSC -> book ->
                            // destination
            {6, "saxfad"},
