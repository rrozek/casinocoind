...
#ifndef CASINOCOIN_APP_PATHS_CASINOCOINSTATE_H_INCLUDED
#define CASINOCOIN_APP_PATHS_CASINOCOINSTATE_H_INCLUDED
...
class CasinocoinState
{
public:
    // VFALCO Why is this shared_ptr?
    using pointer = std::shared_ptr <CasinocoinState>;

public:
    CasinocoinState () = delete;

    virtual ~CasinocoinState() = default;

    static CasinocoinState::pointer makeItem(
        AccountID const& accountID,
        std::shared_ptr<SLE const> sle);

    // Must be public, for make_shared
    CasinocoinState (std::shared_ptr<SLE const>&& sle,
    ...
    bool getNoCasinocoin () const
    {
        return mFlags & (mViewLowest ? lsfLowNoCasinocoin : lsfHighNoCasinocoin);
    }

    bool getNoCasinocoinPeer () const
    {
        return mFlags & (!mViewLowest ? lsfLowNoCasinocoin : lsfHighNoCasinocoin);
    }
...
std::vector <CasinocoinState::pointer>
getCasinocoinStateItems (AccountID const& accountID,
    ReadView const& view);
