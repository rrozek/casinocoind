...
#include <casinocoin/app/paths/PathRequest.h>
...
bool PathRequest::isValid (std::shared_ptr<CasinocoinLineCache> const& crCache)
{
    ...
    if (! sleDest)
    {
        ...
        {
            // Only CSC can be send to a non-existent account.
    ...
    {
        bool const disallowCSC (
            sleDest->getFlags() & lsfDisallowCSC);

        auto usDestCurrID = accountDestCurrencies (
                *raDstAccount, crCache, ! disallowCSC);
...
PathRequest::getPathFinder(std::shared_ptr<CasinocoinLineCache> const& cache,
...
bool
PathRequest::findPaths (std::shared_ptr<CasinocoinLineCache> const& cache,
    int const level, Json::Value& jvArray)
{
        ...
        for (auto const& c : currencies)
        {
            if (! sameAccount || c != saDstAmount.getCurrency())
            {
                ...
                sourceCurrencies.insert(
                    {c, c.isZero() ? cscAccount() : *raSrcAccount});
            }
        }
    }
    ...
    for (auto const& issue : sourceCurrencies)
    {
        ...
        auto& sourceAccount = ! isCSC(issue.account)
            ? issue.account
        ...
        JLOG(m_journal.debug()) << iIdentifier
            << " Paths found, calling casinocoinCalc";
        ...
        if (rc.result () == tesSUCCESS)
        {
            ...
            if (hasCompletion ())
            {
                // Old casinocoin_path_find API requires this
        ...
        {
            JLOG(m_journal.debug()) << iIdentifier << " casinocoinCalc returns "
                << transHuman(rc.result());
        }
    }
...
Json::Value PathRequest::doUpdate(
    std::shared_ptr<CasinocoinLineCache> const& cache, bool fast)
{
    using namespace std::chrono;
    JLOG(m_journal.debug()) << iIdentifier
    ...
    if (hasCompletion ())
    {
        // Old casinocoin_path_find API gives destination_currencies
