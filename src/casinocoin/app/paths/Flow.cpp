...
#include <casinocoin/app/paths/impl/StrandFlow.h>
...
auto finishFlow (PaymentSandbox& sb,
    ...
    FlowResult&& f)
{
    path::CasinocoinCalc::Output result;
    if (f.ter == tesSUCCESS)
        f.sandbox->apply (sb);
    else
    ...
    path::detail::FlowDebugInfo* flowDebugInfo)
{
    Issue const srcIssue = [&] {
        ...
        if (!isCSC (deliver.issue ().currency))
    ...
    const bool srcIsCSC = isCSC (srcIssue.currency);
    const bool dstIsCSC = isCSC (dstIssue.currency);

    auto const asDeliver = toAmountSpec (deliver);

    // The src account may send either csc or iou. The dst account may receive
    // either csc or iou. Since CSC and IOU amounts are represented by different
    // types, use templates to tell `flow` about the amount types.
    if (srcIsCSC && dstIsCSC)
    {
        return finishFlow (sb, srcIssue, dstIssue,
            flow<CSCAmount, CSCAmount> (
                sb, strands, asDeliver.csc, partialPayment, offerCrossing,
    ...
    {
        return finishFlow (sb, srcIssue, dstIssue,
            flow<IOUAmount, CSCAmount> (
                sb, strands, asDeliver.csc, partialPayment, offerCrossing,
                limitQuality, sendMax, j, flowDebugInfo));
    }

    assert (!srcIsCSC && !dstIsCSC);
    return finishFlow (sb, srcIssue, dstIssue,
