...
class CasinocoinCalc; // for logging
...
// - A casinocoin nodes output issuer must be the node's account or the next node's
//   account.
// - Offers can only go directly to another offer if the currency and issuer are
//   an exact match.
// - Real issuers must be specified for non-CSC.
    ...
    AccountID const& issuer)    // --> Delivering this issuer.
{
    ...
    if (nodes_.back ().issue_.currency != currency)
    {
        ...
        // book.  cscAccount() does double duty as signaling "this is an order
        // book".

        // Corresponds to "Implies an offer directory" in the diagram, currently
        // at http://goo.gl/Uj3HAB.

        auto type = isCSC(currency) ? STPathElement::typeCurrency
            : STPathElement::typeCurrency | STPathElement::typeIssuer;

        // The offer's output is what is now wanted.
        // cscAccount() is a placeholder for offers.
        resultCode = pushNode (type, cscAccount(), currency, issuer);
    }


    // For casinocoin, non-CSC, ensure the issuer is on at least one side of the
    // transaction.
    ...
    {
        // Need to casinocoin through issuer's account.
    ...
    AccountID const& issuer)     // If not specified, default to previous.
{
    ...
    // true, iff node is a casinocoin account. false, iff node is an offer node.
    ...
    else if (hasIssuer && isCSC (node.issue_))
    {
        JLOG (j_.debug()) << "pushNode: issuer specified for CSC.";
    ...
    else if (hasAccount)
    {
        ...
        node.issue_.account = hasIssuer ? issuer :
                (isCSC (node.issue_) ? cscAccount() : account);
        ...
        else
        {
            ...
            resultCode = pushImpliedNodes (
                ...
                isCSC(node.issue_.currency) ? cscAccount() : account);
        ...
        if (resultCode == tesSUCCESS && !nodes_.empty ())
        {
            ...
            {
                auto sleCasinocoinState = view().peek(
                    keylet::line(backNode.account_, node.account_, backNode.issue_.currency));

                // A "CasinocoinState" means a balance betweeen two accounts for a
                // specific currency.
    ...
    else
    {
        ...
        else if (isCSC (node.issue_.currency))
            node.issue_.account = cscAccount();
        else if (isCSC (backNode.issue_.account))
        ...
        else if (backNode.issue_ == node.issue_)
        {
            ...
            resultCode   = pushImpliedNodes (
                cscAccount(), // Rippling, but offers don't have an account.
    ...
    AccountID const& uSenderID)
{
    ...
    AccountID const& uSenderIssuerID
        = isCSC(uMaxCurrencyID) ? cscAccount() : uSenderID;
    // Sender is always issuer for non-CSC.
    ...
    // CSC with issuer is malformed.
    if ((isCSC (uMaxCurrencyID) && !isCSC (uMaxIssuerID))
        || (isCSC (currencyOutID) && !isCSC (issuerOutID)))
    {
        JLOG (j_.debug())
            << "expandPath> issuer with CSC";
        terStatus   = temBAD_PATH;
    }

    // Push sending node.
    // For non-CSC, issuer is always sending account.
    // - Trying to expand, not-compact.
    // - Every issuer will be traversed through.
    if (terStatus == tesSUCCESS)
    {
        terStatus   = pushNode (
            !isCSC(uMaxCurrencyID)
    ...
    if (tesSUCCESS == terStatus && uMaxIssuerID != uSenderIssuerID)
    {
        ...
        // - If it was CSC, then issuers would have matched.
        ...
        // account.
        if (!uNxtCurrencyID
            // Next is CSC, offer next. Must go through issuer.
    ...
    if (terStatus == tesSUCCESS
        && !isCSC(currencyOutID)               // Next is not CSC
...
/** Check if a sequence of three accounts violates the no casinocoin constrains
    [first] -> [second] -> [third]
    Disallowed if 'second' set no casinocoin on [first]->[second] and
    [second]->[third]
*/
TER PathState::checkNoCasinocoin (
    ...
    Currency const& currency)
{
    ...
    if (!sleIn || !sleOut)
    {
        ...
        sleIn->getFieldU32 (sfFlags) &
            ((secondAccount > firstAccount) ? lsfHighNoCasinocoin : lsfLowNoCasinocoin) &&
        sleOut->getFieldU32 (sfFlags) &
            ((secondAccount > thirdAccount) ? lsfHighNoCasinocoin : lsfLowNoCasinocoin))
    {
        JLOG (j_.info())
            << "Path violates noCasinocoin constraint between "
            << firstAccount << ", "
            << secondAccount << " and "
            << thirdAccount;

        terStatus = terNO_CASINOCOIN;
    }
    return terStatus;
}

// Check a fully-expanded path to make sure it doesn't violate no-Casinocoin
// settings.
TER PathState::checkNoCasinocoin (
    AccountID const& uDstAccountID,
    AccountID const& uSrcAccountID)
{
    // There must be at least one node for there to be two consecutive casinocoin
    // lines.
    if (nodes_.size() == 0)
       return terStatus;
    ...
    // These are the nodes whose no casinocoin constraints could be violated
