    ...
    , journal_ (journal)
{
    ...
    // flavor we expect:
    assert (cross_type != CrossType::CscToIou ||
        (isCSC (issue_in ()) && !isCSC (issue_out ())));

    assert (cross_type != CrossType::IouToCsc ||
        (!isCSC (issue_in ()) && isCSC (issue_out ())));

    // And make sure we're not crossing CSC for CSC
    assert (!isCSC (issue_in ()) || !isCSC (issue_out ()));
    ...
    Rate const& rate_out)
{
    ...
    // of an autobridge.
    if (cross_type_ == CrossType::CscToIou && (remaining_.in < f.order.in))
    {
    ...
    Rate const& rate_in)
{
    ...
    if (!sell_ && cross_type_ == CrossType::IouToCsc)
    {
...
BasicTaker::do_cross (Amounts offer, Quality quality, AccountID const& owner)
{
    ...
    if (cross_type_ == CrossType::CscToIou)
    {
        result = flow_csc_to_iou (offer, quality, owner_funds, taker_funds,
            out_rate (owner, account ()));
    }
    else if (cross_type_ == CrossType::IouToCsc)
    {
        result = flow_iou_to_csc (offer, quality, owner_funds, taker_funds,
    ...
    Amounts offer2, Quality quality2, AccountID const& owner2)
{
    ...
    // The amount available to flow via CSC is the amount that the owner of the
    ...
    // of CSC that can flow between the two legs is, essentially, infinite
    // since all the owner is doing is taking out CSC of his left pocket
    // and putting it in his right pocket. In that case, we set the available
    // CSC to the largest of the two offers.
    auto csc_funds = get_funds (owner1, offer1.out);

    if (owner1 == owner2)
    {
        JLOG(journal_.trace()) <<
            "The bridge endpoints are owned by the same account.";
        csc_funds = std::max (offer1.out, offer2.in);
    }
    ...
    if (auto stream = journal_.debug())
    {
        ...
        stream << "      csc: " << format_amount (csc_funds);
    }
    ...
    auto flow1 = flow_iou_to_csc (offer1, quality1, csc_funds, leg1_in_funds, leg1_rate);

    if (!flow1.sanity_check ())
        Throw<std::logic_error> ("Computed flow1 fails sanity check.");

    auto flow2 = flow_csc_to_iou (offer2, quality2, leg2_out_funds, csc_funds, leg2_rate);
    ...
    // equalize them, so that the amount of CSC that flows out of the first leg
    // is the same as the amount of CSC that flows into the second leg. We take
...
TER Taker::transferCSC (
    AccountID const& from,
    AccountID const& to,
    STAmount const& amount)
{
    if (!isCSC (amount))
        Throw<std::logic_error> ("Using transferCSC with IOU");
    ...
    return casinocoin::transferCSC (view_, from, to, amount, journal_);
}
...
{
    if (isCSC (amount))
        Throw<std::logic_error> ("Using redeemIOU with CSC");
    ...
    auto ret = casinocoin::redeemIOU (view_, account, amount, issue, journal_);
...
{
    if (isCSC (amount))
        Throw<std::logic_error> ("Using issueIOU with CSC");
    ...
    return casinocoin::issueIOU (view_, account, amount, issue, journal_);
}
...
Taker::fill (BasicTaker::Flow const& flow, Offer& offer)
{
    ...
    if (cross_type () != CrossType::CscToIou)
    {
        assert (!isCSC (flow.order.in));
    ...
    else
    {
        ...
        if (result == tesSUCCESS)
            result = transferCSC (account (), offer.owner (), flow.order.in);
    }

    // Now send funds from the account whose offer we're taking
    if (cross_type () != CrossType::IouToCsc)
    {
        assert (!isCSC (flow.order.out));
    ...
    else
    {
        ...
        if (result == tesSUCCESS)
            result = transferCSC (offer.owner (), account (), flow.order.out);
    }
    ...
    BasicTaker::Flow const& flow2, Offer& leg2)
{
    ...
    // leg1 to leg2: bridging over CSC
    if (result == tesSUCCESS)
        result = transferCSC (leg1.owner (), leg2.owner (), flow1.order.out);
...
{
    // In direct crossings, at least one leg must not be CSC.
    if (isCSC (offer.amount ().in) && isCSC (offer.amount ().out))
        return tefINTERNAL;
...
{
    // In bridged crossings, CSC must can't be the input to the first leg
    // or the output of the second leg.
    if (isCSC (leg1.amount ().in) || isCSC (leg2.amount ().out))
        return tefINTERNAL;
...
Taker::calculateRate (
    ApplyView const& view,
        AccountID const& issuer,
            AccountID const& account)
{
    return isCSC (issuer) || (account == issuer)
        ? parityRate
