...
Payment::calculateMaxSpend(STTx const& tx)
{
    if (tx.isFieldPresent(sfSendMax))
    {
        ...
        return sendMax.native() ? sendMax.csc() : beast::zero;
    }
    /* If there's no sfSendMax in CSC, and the sfAmount isn't
    in CSC, then the transaction can not send CSC. */
...
Payment::preflight (PreflightContext const& ctx)
{
    ...
    bool const defaultPathsAllowed = !(uTxFlags & tfNoCasinocoinDirect);
    ...
    // isZero() is CSC.  FIX!
    bool const bCSCDirect = uSrcCurrency.isZero () && uDstCurrency.isZero ();
    ...
    if (bCSCDirect && bMax)
    {
        ...
        JLOG(j.trace()) << "Malformed transaction: " <<
            "SendMax specified for CSC to CSC.";
        return temBAD_SEND_CSC_MAX;
    }
    if (bCSCDirect && bPaths)
    {
        // CSC is sent without paths.
        JLOG(j.trace()) << "Malformed transaction: " <<
            "Paths specified for CSC to CSC.";
        return temBAD_SEND_CSC_PATHS;
    }
    if (bCSCDirect && partialPaymentAllowed)
    {
        // Consistent but redundant transaction.
        JLOG(j.trace()) << "Malformed transaction: " <<
            "Partial payment specified for CSC to CSC.";
        return temBAD_SEND_CSC_PARTIAL;
    }
    if (bCSCDirect && limitQuality)
    {
        // Consistent but redundant transaction.
        JLOG(j.trace()) << "Malformed transaction: " <<
            "Limit quality specified for CSC to CSC.";
        return temBAD_SEND_CSC_LIMIT;
    }
    if (bCSCDirect && !defaultPathsAllowed)
    {
        // Consistent but redundant transaction.
        JLOG(j.trace()) << "Malformed transaction: " <<
            "No casinocoin direct specified for CSC to CSC.";
        return temBAD_SEND_CSC_NO_DIRECT;
    }
...
{
    // Casinocoin if source or destination is non-native or if there are paths.
...
Payment::doApply ()
{
    ...
    if (bCasinocoin)
    {
        ...
        path::CasinocoinCalc::Output rc;
        {
            PaymentSandbox pv(&view());
            JLOG(j_.debug())
                << "Entering CasinocoinCalc in payment: " << ctx_.tx.getTransactionID();
        ...
        // Because of its overhead, if CasinocoinCalc
    ...
    else
    {
        ...
        auto const mmm = std::max(reserve,
            ctx_.tx.getFieldAmount (sfFee).csc ());

        if (mPriorBalance < saDstAmount.csc () + mmm)
        {
            // Vote no. However the transaction might succeed, if applied in
            // a different order.
            JLOG(j_.trace()) << "Delay transaction: Insufficient funds: " <<
                " " << to_string (mPriorBalance) <<
                " / " << to_string (saDstAmount.csc () + mmm) <<
                " (" << to_string (reserve) << ")";
