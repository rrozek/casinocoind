...
#ifndef CASINOCOIN_APP_BOOK_TAKER_H_INCLUDED
#define CASINOCOIN_APP_BOOK_TAKER_H_INCLUDED

#include <casinocoin/app/tx/impl/Offer.h>
...
private:
    ...
    Flow
    flow_csc_to_iou (Amounts const& offer, Quality quality,
        STAmount const& owner_funds, STAmount const& taker_funds,
        Rate const& rate_out);

    Flow
    flow_iou_to_csc (Amounts const& offer, Quality quality,
        STAmount const& owner_funds, STAmount const& taker_funds,
        Rate const& rate_in);
...
private:
    ...
    TER
    transferCSC (AccountID const& from, AccountID const& to, STAmount const& amount);
...
private:
    ...
    // The amount of CSC that flowed if we were autobridging
    STAmount csc_flow_;
