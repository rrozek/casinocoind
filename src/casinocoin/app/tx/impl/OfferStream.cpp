...
static
CSCAmount accountFundsHelper (ReadView const& view,
    AccountID const& id,
    CSCAmount const& amtDefault,
    Issue const& issue,
    FreezeHandling freezeHandling,
    beast::Journal j)
{
    return toAmount<CSCAmount> (
        accountHolds (view, id, issue.currency, issue.account, freezeHandling, j));
}
...
template class FlowOfferStream<CSCAmount, IOUAmount>;
template class FlowOfferStream<IOUAmount, CSCAmount>;

template class TOfferStreamBase<STAmount, STAmount>;
template class TOfferStreamBase<IOUAmount, IOUAmount>;
template class TOfferStreamBase<CSCAmount, IOUAmount>;
template class TOfferStreamBase<IOUAmount, CSCAmount>;
}
