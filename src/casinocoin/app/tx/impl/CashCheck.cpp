...
CashCheck::preclaim (PreclaimContext const& ctx)
{
    ...
    }
    {
        ...
        // less than value the check cannot be cashed.
        {
            ...
            // Note that src will have one reserve's worth of additional CSC
            // once the check is cashed, since the check's reserve will no
            // longer be required.  So, if we're dealing in CSC, we add one
            // reserve's worth to the available funds.
            if (value.native())
                availableFunds += CSCAmount (ctx.view.fees().increment);
...
CashCheck::doApply ()
{
    ...
    if (srcId != account_)
    {
        ...
        // Flow() doesn't do CSC to CSC transfers.
        if (sendMax.native())
        {
            // Here we need to calculate the amount of CSC sleSrc can send.
            ...
            // argument.
            STAmount const srcLiquid {cscLiquid (psb, srcId, -1, viewJ)};

            // Now, how much do they need in order to be successful?
            STAmount const cscDeliver {
            ...
            if (srcLiquid < cscDeliver)
            {
                // Vote no. However the transaction might succeed if applied
                // in a different order.
                JLOG(j_.trace()) << "Cash Check: Insufficient CSC: "
                    << srcLiquid.getFullText()
                    << " < " << cscDeliver.getFullText();
                return tecUNFUNDED_PAYMENT;
            }
            else
            {
                // The source account has enough CSC so make the ledger change.
                transferCSC (psb, srcId, account_, cscDeliver, viewJ);
            }
        }
        else
        {
