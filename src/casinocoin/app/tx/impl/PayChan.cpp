...
/*
    PaymentChannel
        ...
        Payment channels permit off-ledger checkpoints of CSC payments flowing
        in a single direction. A channel sequesters the owner's CSC in its own
    ...
    PaymentChannelCreate
        ...
        Amount
            The amount of CSC to deposit in the channel immediately.
        SettleDelay
    ...
    PaymentChannelClaim
        ...
        Channel
            The 256-bit ID of the channel.
        Balance (optional)
            The total amount of CSC delivered after this claim is processed (optional, not
            needed if just closing).
        Amount (optional)
            The amount of CSC the signature is for (not needed if equal to Balance or just
            closing the line).
        Signature (optional)
...
PayChanCreate::preflight (PreflightContext const& ctx)
{
    ...
    if (!isCSC (ctx.tx[sfAmount]) || (ctx.tx[sfAmount] <= beast::zero))
...
PayChanClaim::preflight (PreflightContext const& ctx)
{
    ...
    if (bal && (!isCSC (*bal) || *bal <= beast::zero))
        return temBAD_AMOUNT;

    auto const amt = ctx.tx[~sfAmount];
    if (amt && (!isCSC (*amt) || *amt <= beast::zero))
        return temBAD_AMOUNT;
...
PayChanClaim::doApply()
{
    ...
    {
        auto const chanBalance = slep->getFieldAmount (sfBalance).csc ();
        auto const chanFunds = slep->getFieldAmount (sfAmount).csc ();
        auto const reqBalance = ctx_.tx[sfBalance].csc ();
        ...
        if (txAccount == src && (sled->getFlags() & lsfDisallowCSC))
            return tecNO_TARGET;
        ...
        CSCAmount const reqDelta = reqBalance - chanBalance;
