...
SetTrust::preflight (PreflightContext const& ctx)
{
    ...
    {
        JLOG(j.trace()) <<
            "Malformed transaction: specifies CSC as IOU";
...
SetTrust::doApply ()
{
    ...
    // which needs trust lines, is a powerful Casinocoin feature.
    ...
    // user enough CSC to cover not only the account reserve
    ...
    bool const bSetAuth = (uTxFlags & tfSetfAuth);
    bool const bSetNoCasinocoin = (uTxFlags & tfSetNoCasinocoin);
    bool const bClearNoCasinocoin  = (uTxFlags & tfClearNoCasinocoin);
    ...
    SLE::pointer sleCasinocoinState = view().peek (
        keylet::line(account_, uDstAccountID, currency));

    if (sleCasinocoinState)
    {
        ...
        saLowBalance    = sleCasinocoinState->getFieldAmount (sfBalance);
        ...
        sleCasinocoinState->setFieldAmount (!bHigh ? sfLowLimit : sfHighLimit, saLimitAllow);

        saLowLimit  = !bHigh ? saLimitAllow : sleCasinocoinState->getFieldAmount (sfLowLimit);
        saHighLimit =  bHigh ? saLimitAllow : sleCasinocoinState->getFieldAmount (sfHighLimit);
        ...
        if (!bQualityIn)
        {
            ...
            uLowQualityIn   = sleCasinocoinState->getFieldU32 (sfLowQualityIn);
            uHighQualityIn  = sleCasinocoinState->getFieldU32 (sfHighQualityIn);
        }
        else if (uQualityIn)
        {
            // Setting.

            sleCasinocoinState->setFieldU32 (!bHigh ? sfLowQualityIn : sfHighQualityIn, uQualityIn);

            uLowQualityIn   = !bHigh ? uQualityIn : sleCasinocoinState->getFieldU32 (sfLowQualityIn);
            uHighQualityIn  =  bHigh ? uQualityIn : sleCasinocoinState->getFieldU32 (sfHighQualityIn);
        }
        else
        {
            // Clearing.

            sleCasinocoinState->makeFieldAbsent (!bHigh ? sfLowQualityIn : sfHighQualityIn);

            uLowQualityIn   = !bHigh ? 0 : sleCasinocoinState->getFieldU32 (sfLowQualityIn);
            uHighQualityIn  =  bHigh ? 0 : sleCasinocoinState->getFieldU32 (sfHighQualityIn);
        }
        ...
        if (!bQualityOut)
        {
            ...
            uLowQualityOut  = sleCasinocoinState->getFieldU32 (sfLowQualityOut);
            uHighQualityOut = sleCasinocoinState->getFieldU32 (sfHighQualityOut);
        }
        else if (uQualityOut)
        {
            // Setting.

            sleCasinocoinState->setFieldU32 (!bHigh ? sfLowQualityOut : sfHighQualityOut, uQualityOut);

            uLowQualityOut  = !bHigh ? uQualityOut : sleCasinocoinState->getFieldU32 (sfLowQualityOut);
            uHighQualityOut =  bHigh ? uQualityOut : sleCasinocoinState->getFieldU32 (sfHighQualityOut);
        }
        else
        {
            // Clearing.

            sleCasinocoinState->makeFieldAbsent (!bHigh ? sfLowQualityOut : sfHighQualityOut);

            uLowQualityOut  = !bHigh ? 0 : sleCasinocoinState->getFieldU32 (sfLowQualityOut);
            uHighQualityOut =  bHigh ? 0 : sleCasinocoinState->getFieldU32 (sfHighQualityOut);
        }
        ...
        bool const bLowDefCasinocoin        = sleLowAccount->getFlags() & lsfDefaultCasinocoin;
        bool const bHighDefCasinocoin       = sleHighAccount->getFlags() & lsfDefaultCasinocoin;
        ...
        if (uFlagsIn != uFlagsOut)
            sleCasinocoinState->setFieldU32 (sfFlags, uFlagsOut);
        ...
        if (bDefault || badCurrency() == currency)
        {
            ...
            terResult = trustDelete (view(),
                sleCasinocoinState, uLowAccountID, uHighAccountID, viewJ);
        }
        ...
        else if (bReserveIncrease && mPriorBalance < reserveCreate)
        {
            ...
            // Another transaction could provide CSC to the account and then
            // this transaction would succeed.
            terResult = tecINSUF_RESERVE_LINE;
        }
        else
        {
            view().update (sleCasinocoinState);

            JLOG(j_.trace()) << "Modify casinocoin line";
        }
    }
    // Line does not exist.
    ...
    {
        JLOG(j_.trace()) <<
            "Redundant: Setting non-existent casinocoin line to defaults.";
    ...
    else
    {
        ...
        uint256 index (getCasinocoinStateIndex (
            account_, uDstAccountID, currency));

        JLOG(j_.trace()) <<
            "doTrustSet: Creating casinocoin line: " <<
            to_string (index);

        // Create a new casinocoin line.
        terResult = trustCreate (view(),
            ...
            sle,
            bSetAuth,
            bSetNoCasinocoin && !bClearNoCasinocoin,
