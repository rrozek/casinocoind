...
/*
    ...
    2018-01-10  jrojek          Added KYC Verification
*/
//==============================================================================
...
SetAccount::preflight (PreflightContext const& ctx)
{
    ...
    if (bSetDisallowCSC && bClearDisallowCSC)
    {
        JLOG(j.trace()) << "Malformed transaction: Contradictory flags set.";
        return temINVALID_FLAG;
    }
...
SetAccount::doApply ()
{
    ...
    // DisallowCSC
    //
    if (bSetDisallowCSC && !(uFlagsIn & lsfDisallowCSC))
    {
        JLOG(j_.trace()) << "Set lsfDisallowCSC.";
        uFlagsOut |= lsfDisallowCSC;
    }

    if (bClearDisallowCSC && (uFlagsIn & lsfDisallowCSC))
    {
        JLOG(j_.trace()) << "Clear lsfDisallowCSC.";
        uFlagsOut &= ~lsfDisallowCSC;
    }
