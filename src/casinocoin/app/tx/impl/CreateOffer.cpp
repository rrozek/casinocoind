...
CreateOffer::preflight (PreflightContext const& ctx)
{
    ...
    // We don't allow a non-native currency to use the currency code CSC.
...
CreateOffer::reachedOfferCrossingLimit (Taker const& taker) const
{
    ...
    // The crossing limit is part of the Casinocoin protocol and
    ...
    NetClock::time_point const when)
{
    ...
    assert (!isCSC (takerAmount.in) && !isCSC (takerAmount.out));

    if (isCSC (takerAmount.in) || isCSC (takerAmount.out))
        Throw<std::logic_error> ("Bridging with CSC and an endpoint.");
    ...
    OfferStream offers_leg1 (view, view_cancel,
        Book (taker.issue_in (), cscIssue ()),
        when, stepCounter_, j_);

    OfferStream offers_leg2 (view, view_cancel,
        Book (cscIssue (), taker.issue_out ()),
        when, stepCounter_, j_);
    ...
    Amounts const& takerAmount)
{
    ...
    //
    // We check this in preclaim, but when selling CSC
    ...
    Amounts const& takerAmount)
{
    try
    {
        ...
        //
        // We check this in preclaim, but when selling CSC charged fees can
        ...
        // of the takerAmount currencies are CSC then we cross through an
        // additional path with CSC as the intermediate between two books.
...
CreateOffer::preCompute()
{
    ...
    if (pays_csc && !gets_csc)
        cross_type_ = CrossType::IouToCsc;
    else if (gets_csc && !pays_csc)
        cross_type_ = CrossType::CscToIou;
...
CreateOffer::applyGuts (Sandbox& sb, Sandbox& sbCancel)
{
    ...
    {
        CSCAmount reserve = ctx_.view().fees().accountReserve(
