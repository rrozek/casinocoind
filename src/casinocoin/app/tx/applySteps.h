...
#ifndef CASINOCOIN_TX_APPLYSTEPS_H_INCLUDED
#define CASINOCOIN_TX_APPLYSTEPS_H_INCLUDED
...
struct TxConsequences
{
    ...
    CSCAmount const potentialSpend;

    /// Constructor
    TxConsequences(ConsequenceCategory const category_,
        CSCAmount const fee_, CSCAmount const spend_)
        : category(category_)
        , fee(fee_)
...
/** Determine the CSC balance consequences if a transaction
    consumes the maximum CSC allowed.
