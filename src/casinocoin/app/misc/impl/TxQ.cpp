    ...
    TxQ::Setup const& setup)
{
    ...
    // happen.
    return mulDiv(tx[sfFee].csc().drops(),
        baseRefLevel,
...
/*
    ...
    0. Is `featureFeeEscalation` enabled?
        ...
        No: Fallback to `casinocoin::apply`. Stop.
        ...
        ApplyFlags flags, beast::Journal j)
{
    ...
    struct MultiTxn
    {
        ...
        CSCAmount fee = beast::zero;
        CSCAmount potentialSpend = beast::zero;
    ...
    {
        auto feeLevel = FeeMetrics::scaleFeeLevel(j_, metricsSnapshot, view);
        ...
        if (sle)
        {
            ...
            if (multiTxn)
            {
                /* Check if the total fees in flight are greater
                    ...
                    Assume: Minimum account reserve is 20 CSC.
                    Example 1: If I have 1,000,000 CSC, I can queue
                        a transaction with a 1,000,000 CSC fee. In
                        the meantime, some other transaction may
                        lower my balance (eg. taking an offer). When
                        the transaction executes, I will either
                        spend the 1,000,000 CSC, or the transaction
                        will get stuck in the queue with a
                        `terINSUF_FEE_B`.
                    Example 2: If I have 1,000,000 CSC, and I queue
                        10 transactions with 0.1 CSC fee, I have 1 CSC
                        in flight. I can now queue another tx with a
                        999,999 CSC fee. When the first 10 execute,
                        ...
                        999,999 CSC, or get stuck in the queue.
                    Example 3: If I have 1,000,000 CSC, and I queue
                        7 transactions with 3 CSC fee, I have 21 CSC
...
TxQ::getMetrics(OpenView const& view) const
{
    ...
    JLOG(j_.debug()) << "TxQ::getMetrics. txCount " << result.txCount <<
                       ...
                       " minFeeLevel " << result.minProcessingFeeLevel <<
                       " medFeeLevel " << result.medFeeLevel <<
                       " expFeeLevel " << result.openLedgerFeeLevel;
    return result;
}
