              ...
              beast::Journal j)
{
    ...
    if (isCSC(currency))
    {
        return {cscLiquid (view, account, 0, j)};
    }
...
CSCAmount
cscLiquid (ReadView const& view, AccountID const& id,
    std::int32_t ownerCountAdj, beast::Journal j)
{
    ...
    if (fix1141 (view.info ().parentCloseTime))
    {
        ...
        auto const balance = view.balanceHook(id, cscAccount(), fullBalance);
    ...
    {
        // pre-switchover
        // CSC: return balance minus reserve
        ...
        return view.balanceHook(id, cscAccount(), amount).csc();
    }
}
...
trustCreate (ApplyView& view,
    ...
    const bool      bnoCasinocoin,          // --> others cannot ripple through
    ...
    beast::Journal j)
{
    ...
    auto const sleCasinocoinState = std::make_shared<SLE>(
...
trustDelete (ApplyView& view,
    std::shared_ptr<SLE> const& sleCasinocoinState,
...
TER
casinocoinCredit (ApplyView& view,
    ...
    beast::Journal j)
{
    ...
    uint256 uIndex = getCasinocoinStateIndex (
        uSenderID, uReceiverID, saAmount.getCurrency ());
    auto sleCasinocoinState  = view.peek (keylet::line(uIndex));

    TER terResult;

    assert (!isCSC (uSenderID) && uSenderID != noAccount());
    assert (!isCSC (uReceiverID) && uReceiverID != noAccount());
    ...
    if (!sleCasinocoinState)
    {
        ...
        bool noCasinocoin = (sleAccount->getFlags() & lsfDefaultCasinocoin) == 0;
    ...
    {
        STAmount saBalance   = sleCasinocoinState->getFieldAmount (sfBalance);
        ...
        std::uint32_t const uFlags (sleCasinocoinState->getFieldU32 (sfFlags));
        bool bDelete = false;
        ...
        if (saBefore > beast::zero
            ...
            && static_cast <bool> (uFlags & (!bSenderHigh ? lsfLowNoCasinocoin : lsfHighNoCasinocoin)) !=
               static_cast <bool> (view.read (keylet::account(uSenderID))->getFlags() & lsfDefaultCasinocoin)
            && !(uFlags & (!bSenderHigh ? lsfLowFreeze : lsfHighFreeze))
            && !sleCasinocoinState->getFieldAmount (
                !bSenderHigh ? sfLowLimit : sfHighLimit)
            // Sender trust limit is 0.
            && !sleCasinocoinState->getFieldU32 (
        ...
        sleCasinocoinState->setFieldAmount (sfBalance, saBalance);
    ...
    STAmount const& saAmount, STAmount& saActual, beast::Journal j)
{
    ...
    assert (!isCSC (uSenderID) && !isCSC (uReceiverID));
    assert (uSenderID != uReceiverID);

    if (uSenderID == issuer || uReceiverID == issuer || issuer == noAccount())
    {
        // Direct send: redeeming IOUs and/or sending own IOUs.
        casinocoinCredit (view, uSenderID, uReceiverID, saAmount, false, j);
        saActual = saAmount;
        return tesSUCCESS;
    }
    ...
    TER terResult   = casinocoinCredit (view, issuer, uReceiverID, saAmount, true, j);

    if (tesSUCCESS == terResult)
        terResult = casinocoinCredit (view, uSenderID, issuer, saActual, true, j);
    ...
    STAmount const& saAmount, beast::Journal j)
{
    ...
    /* CSC send which does not check reserve and can do pure adjustment.
    ...
    if (sender)
    {
        ...
        else
        {
            ...
            if (fv2Switch)
                view.creditHook (uSenderID, cscAccount (), saAmount, sndBal);
    ...
    {
        // Increment CSC balance.
        auto const rcvBal = receiver->getFieldAmount (sfBalance);
        receiver->setFieldAmount (sfBalance, rcvBal + saAmount);

        if (fv2Switch)
            view.creditHook (cscAccount (), uReceiverID, saAmount, -rcvBal);
    ...
    beast::Journal j)
{
    ...
    if (before > beast::zero
        ...
        // Sender reserve is set.
        && static_cast <bool> (flags & (!bSenderHigh ? lsfLowNoCasinocoin : lsfHighNoCasinocoin)) !=
           static_cast <bool> (sle->getFlags() & lsfDefaultCasinocoin)
...
{
    assert (!isCSC (account) && !isCSC (issue.account));
    ...
    uint256 const index = getCasinocoinStateIndex (
    ...
    if (!state)
    {
        ...
        bool noCasinocoin = (receiverAccount->getFlags() & lsfDefaultCasinocoin) == 0;

        return trustCreate (view, bSenderHigh, issue.account, account, index,
            receiverAccount, false, noCasinocoin, false, final_balance, limit, 0, 0, j);
    }
