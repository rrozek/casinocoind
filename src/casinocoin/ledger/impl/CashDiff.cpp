...
struct CashSummary
{
    ...
    void sort()
    {
        std::sort (cscChanges.begin(), cscChanges.end());
    ...
    std::shared_ptr<SLE const> const& after)
{
    if (isDelete)
    {
        ...
        {
        case ltACCOUNT_ROOT:
            result.cscChanges.push_back (
                std::make_pair (prev[sfAccount], CSCAmount {0}));
            return true;
    ...
    else
    {
        ...
        case ltACCOUNT_ROOT:
        {
            auto const curCsc = cur[sfBalance].csc();
            if (!before || (*before)[sfBalance].csc() != curCsc)
                result.cscChanges.push_back (
                    std::make_pair (cur[sfAccount], curCsc));
            return true;
        }
        case ltCASINOCOIN_STATE:
        {
...
{
private:
    // Note differences in destroyed CSC between two ApplyStateTables.
    struct DropsGone
    {
        CSCAmount lhs;
        CSCAmount rhs;
...
countKeys (detail::CashSummary const& lhs, detail::CashSummary const& rhs)
{
    ...
    addIn (countKeys(lhs.cscChanges,     rhs.cscChanges));
...
bool CashDiff::Impl::rmDust ()
{
    ...
    // amount differences: cscChanges, trustChanges, offerChanges, and
    // offerDeletions.  Rifle through those containers and remove any
    // entries that are _almost_ the same between lhs and rhs.

    // cscChanges.  We call a difference of 2 drops or less dust.
    removedDust |= rmVecDust (lhsDiffs_.cscChanges, rhsDiffs_.cscChanges,
        [](CSCAmount const& lhs, CSCAmount const& rhs)
        {
        ...
        CashFilter rhsFilter, detail::ApplyStateTable const& rhs)
{
    ...
    // cscChanges:
    setDiff (lhsDiffs.cscChanges, rhsDiffs.cscChanges, lhsDiffs_.cscChanges);
    setDiff (rhsDiffs.cscChanges, lhsDiffs.cscChanges, rhsDiffs_.cscChanges);
...
bool diffIsDust (STAmount const& v1, STAmount const& v2, std::uint8_t e10)
{
    ...
    // Handling CSC is different from IOU.
    if (v1.native())
    {
