...
public:
    ...
    {
        {
            ...
            Issue const csc (cscIssue ());

            // fractional CSC (i.e. drops)
            testSetValue ("1", csc);
            testSetValue ("22", csc);
            testSetValue ("333", csc);
            ...
            testSetValue ("100000000000", csc);
            testSetValue ("1000000000000", csc);
            testSetValue ("10000000000000", csc);
            testSetValue ("100000000000000", csc);
    ...
    void testNativeCurrency ()
    {
        ...
        unexpected (to_string (Currency ()) != "CSC", "cHC(CSC)");
        Currency c;
    ...
    void testCustomCurrency ()
    {
        ...
        unexpected (multiply (STAmount (noIssue(), 20), STAmount (3), cscIssue ()).getText () != "60",
            "STAmount multiply fail 2");
        unexpected (multiply (STAmount (20), STAmount (3), noIssue()).getText () != "60",
            "STAmount multiply fail 3");
        unexpected (multiply (STAmount (20), STAmount (3), cscIssue ()).getText () != "60",
        ...
        unexpected (divide (STAmount (noIssue(), 60), STAmount (3), cscIssue ()).getText () != "20",
            "STAmount divide fail");

        unexpected (divide (STAmount (noIssue(), 60), STAmount (noIssue(), 3), noIssue()).getText () != "20",
            "STAmount divide fail");

        unexpected (divide (STAmount (noIssue(), 60), STAmount (noIssue(), 3), cscIssue ()).getText () != "20",
            "STAmount divide fail");
    ...
    void testUnderflow ()
    {
        ...
        bigDsmall = divide (smallValue, bigValue, cscIssue ());

        BEAST_EXPECT(bigDsmall == beast::zero);

        bigDsmall = divide (smallValue, bigNative, cscIssue ());
    ...
    void testRounding ()
    {
        ...
        STAmount dripTest1 = mulRound (twoThird2, two, cscIssue (), false);
        STAmount dripTest2 = multiply (twoThird2, two, cscIssue ());
        STAmount dripTest3 = mulRound (twoThird2, two, cscIssue (), true);
        log << dripTest1;
        log << dripTest2;
        log << dripTest3;
#endif
    ...
    testConvertCSC ()
    {
        testcase ("STAmount to CSCAmount conversions");
        ...
        {
            auto const t = amountFromString (csc, std::to_string (drops));
            auto const s = t.csc ();
            BEAST_EXPECT(s.drops() == drops);
            BEAST_EXPECT(t == STAmount (CSCAmount (drops)));
            BEAST_EXPECT(s == CSCAmount (drops));
        }

        try
        {
            auto const t = amountFromString (usd, "136500");
            fail (to_string (t.csc ()));
        }
    ...
    testConvertIOU ()
    {
        ...
        try
        {
            auto const t = amountFromString (csc, "136500");
