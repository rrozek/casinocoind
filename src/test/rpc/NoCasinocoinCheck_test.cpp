...
#include <casinocoin/resource/impl/Tuning.h>
#include <casinocoin/rpc/impl/Tuning.h>

namespace casinocoin {

class noCasinocoinCheck_test : public beast::unit_test::suite
{
    void
    testBadInput ()
    {
        testcase ("Bad input to noCasinocoin_check");
        ...
        { // missing account field
            auto const result = env.rpc ("json", "noCasinocoin_check", "{}")
                [jss::result];
    ...
    {
        testcase << "Request noCasinocoin_check for " <<
        ...
        {
            env (fset (alice, asfDefaultCasinocoin));
            env (trust (alice, gw["USD"](100)));
        }
        else
        {
            env (fclear (alice, asfDefaultCasinocoin));
            env (trust (alice, gw["USD"](100), gw, tfSetNoCasinocoin));
        }
        env.close ();
        ...
        params[jss::ledger] = "current";
        auto result = env.rpc ("json", "noCasinocoin_check",
            boost::lexical_cast<std::string>(params)) [jss::result];
        ...
        result = env.rpc ("json", "noCasinocoin_check",
            boost::lexical_cast<std::string>(params)) [jss::result];
...
class noCasinocoinCheckLimits_test : public beast::unit_test::suite
{
    void
    testLimits(bool admin)
    {
        ...
        auto checkBalance = [&env]()
        {
            ...
            // cases.
            using namespace casinocoin::Resource;
        ...
        for (auto i = 0; i < casinocoin::RPC::Tuning::noCasinocoinCheck.rmax + 5; ++i)
        {
            if (! admin)
                checkBalance();
...
BEAST_DEFINE_TESTSUITE_MANUAL_PRIO(noCasinocoinCheckLimits, app, casinocoin, 1);
