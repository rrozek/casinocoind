...
class OwnerInfo_test : public beast::unit_test::suite
{
    ...
    testBasic ()
    {
        ...
        env(offer(alice, USD(1), CSC(1000)));
        env.close();

        env(pay(gw, alice, USD(50)));
        env(pay(gw, alice, CNY(50)));
        env(offer(alice, CNY(2), CSC(1000)));
        ...
        if (! BEAST_EXPECT (result[jss::accepted].isMember(jss::casinocoin_lines)))
            return;
        auto lines = result[jss::accepted][jss::casinocoin_lines];
        ...
        BEAST_EXPECT (
            offers[0u][sfTakerGets.fieldName] == CSC(1000).value().getJson(0));
        BEAST_EXPECT (
            offers[0u][sfTakerPays.fieldName] == USD(1).value().getJson(0));


        // current ledger entry
        if (! BEAST_EXPECT (result[jss::current].isMember(jss::casinocoin_lines)))
            return;
        lines = result[jss::current][jss::casinocoin_lines];
        if (! BEAST_EXPECT (lines.isArray() && lines.size() == 2))
            return;
        ...
        BEAST_EXPECT (
            offers[1u][sfTakerGets.fieldName] == CSC(1000).value().getJson(0));
        BEAST_EXPECT (
