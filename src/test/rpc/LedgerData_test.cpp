...
public:
    ...
    void testLedgerType()
    {
        ...
        env(offer(Account{"bob0"}, USD(100), CSC(100)));
...

        {
            ...
            jv[jss::Amount] = CSC(50).value().getJson(0);
...

        {
            ...
            jv[jss::Amount] = CSC(100).value().getJson (0);
        ...
        BEAST_EXPECT( checkArraySize(jrr[jss::state], 1) );
        for (auto const& j : jrr[jss::state])
            BEAST_EXPECT( j["LedgerEntryType"] == jss::CasinocoinState );
        }
...
BEAST_DEFINE_TESTSUITE_PRIO(LedgerData,app,casinocoin,1);
