...
public:
    ...
    testOneSideEmptyBook()
    {
...

        {
            ...
            books[jss::books] = Json::arrayValue;
            {
                ...
                j[jss::snapshot] = true;
                j[jss::taker_gets][jss::currency] = "CSC";
            ...
            if (wsc->version() == 2)
            {
                ...
                BEAST_EXPECT(jv.isMember(jss::casinocoinrpc) && jv[jss::casinocoinrpc] == "2.0");
...

        {
            ...
            env(offer("alice", CSC(700), USD(100)),
                ...
                [&](auto const& jv)
                {
                    ...
                    return t[jss::TransactionType] == jss::OfferCreate &&
                        t[jss::TakerGets] == USD(100).value().getJson(0) &&
                            t[jss::TakerPays] == CSC(700).value().getJson(0);
                }));
        }

        {
            // Create a bid: TakerPays 100/USD, TakerGets 75
            env(offer("alice", USD(100), CSC(75)),
    ...
    testOneSideOffersInBook()
    {
        ...
        env(offer("alice", USD(100), CSC(200)),
            require(owners("alice", 2)));
        env.close();
...

        {
            ...
            BEAST_EXPECT(wsc->findMsg(5s,
                [&](auto const& jv)
                {
                    auto const& t = jv[jss::transaction];
                    return t[jss::TransactionType] == jss::OfferCreate &&
                        t[jss::TakerGets] == CSC(75).value().getJson(0) &&
    ...
    testTrackOffers()
    {
        ...
        Account bob {"bob"};
        auto wsc = makeWSClient(env.app().config());
        env.fund(CSC(20000), alice, bob, gw);
        env.close();
        auto USD = gw["USD"];
        ...
        env(offer(alice, CSC(4000), USD(10)));
        env.close();

        Json::Value jvParams;
        jvParams[jss::taker] = env.master.human();
        jvParams[jss::taker_pays][jss::currency] = "CSC";
        ...
        BEAST_EXPECT(wsc->findMsg(5s,
            [&](auto const& jv)
            {
                ...
                return t[jss::TransactionType] == jss::OfferCreate &&
                       ...
                       t[jss::TakerPays] == CSC(4000).value().getJson(0);
            }));

        env(offer(bob, CSC(2000), USD(5)));
        env.close();
        ...
        BEAST_EXPECT(wsc->findMsg(5s,
            [&](auto const& jv)
            {
                ...
                return t[jss::TransactionType] == jss::OfferCreate &&
                       ...
                       t[jss::TakerPays] == CSC(2000).value().getJson(0);
            }));
    ...
    testCrossingSingleBookOffer()
    {
        ...
        // Scenario is:
        //  - Alice and Bob place identical offers for USD -> CSC
        ...
        env.fund (CSC(1000000), gw, alice, bob, charlie);
        env.close();
        ...
        // Alice and Bob offer $500 for 500 CSC
        env (offer (alice, CSC(500), USD(500)));
        env (offer (bob, CSC(500), USD(500)));
        env.close();
        ...
        env(offer(charlie, USD(1000), CSC(1000)));
        env.close();
        env.require(offers(alice, 0), offers(bob, 0), offers(charlie, 0));
        using namespace std::chrono_literals;
        BEAST_EXPECT(offerOnlyOnceInStream(wsc, 1s, CSC(1000), USD(1000)));
    ...
    testCrossingMultiBookOffer()
    {
        ...
        // Scenario is:
        //  - Alice has 1 USD and wants 100 CSC
        //  - Bob has 100 CSC and wants 1 EUR
        ...
        env(offer(alice, CSC(100), USD(1)));
        env(offer(bob, EUR(1), CSC(100)));
        env.close();
...

        {
...

            {
                ...
                j[jss::taker_pays][jss::currency] = "CSC";
            }
    ...
    testBookOfferErrors()
    {
...

        {
            ...
            jvParams[jss::taker_gets][jss::currency] = "CSC";
            auto const jrr = env.rpc(
                "json", "book_offers", to_string(jvParams)) [jss::result];
...

        {
            ...
            BEAST_EXPECT(jrr[jss::error_message] ==
                ...
                "for CSC currency specification.");
        }

        {
            ...
            jvParams[jss::taker_pays][jss::issuer]   = toBase58(cscAccount());
            jvParams[jss::taker_gets][jss::currency] = "USD";
            jvParams[jss::taker_gets][jss::issuer]   = gw.human();
            auto const jrr = env.rpc(
                "json", "book_offers", to_string(jvParams)) [jss::result];
            BEAST_EXPECT(jrr[jss::error] == "srcIsrMalformed");
            BEAST_EXPECT(jrr[jss::error_message] ==
                "Invalid field 'taker_pays.issuer', expected non-CSC issuer.");
        }
    ...
    testBookOfferLimits(bool asAdmin)
    {
        ...
        Account gw {"gw"};
        env.fund(CSC(200000), gw);
        env.close();
        auto USD = gw["USD"];

        for(auto i = 0; i <= RPC::Tuning::bookOffers.rmax; i++)
            env(offer(gw, CSC(50 + 1*i), USD(1.0 + 0.1*i)));
        env.close();
...
BEAST_DEFINE_TESTSUITE_PRIO(Book,app,casinocoin,1);
