...
class Check_test : public beast::unit_test::suite
{
    ...
    void testEnabled()
    {
        ...
        Account const alice {"alice"};
        {
            ...
            env (check::create (env.master, alice, CSC(100)),
                ter(temDISABLED));
            env.close();

            env (check::cash (alice, checkId, CSC(100)),
                ter (temDISABLED));
            env.close();
        ...
        }
        {
            ...
            env (check::cash (alice, checkId1, CSC(100)));
            env.close();
    ...
    void testCreateValid()
    {
        ...
        STAmount const startBalance {CSC(1000).value()};
            ...
            [&env, &USD, this] (Account const& from, Account const& to)
        {
            ...
            env (check::create (from, to, CSC(2000)));
            env.close();
    ...
    void testCreateInvalid()
    {
        ...
        // Check to self.
        env (check::create (alice, alice, CSC(10)), ter (temREDUNDANT));
        env.close();
    ...
    void testCashCSC()
    {
        // Explore many of the valid ways to cash a check for CSC.
        testcase ("Cash CSC");
        ...
        std::uint64_t const baseFeeDrops {env.current()->fees().base};
        STAmount const startBalance {CSC(300).value()};
        env.fund (startBalance, alice, bob);
        {
            ...
            env (check::cash (bob, chkId, CSC(10)));
            env.close();
            env.require (balance (alice,
                startBalance - CSC(10) - drops (baseFeeDrops)));
            ...
            env (pay (env.master, alice, CSC(10) + drops (baseFeeDrops)));
            env (pay (bob, env.master,   CSC(10) - drops (baseFeeDrops * 2)));
            env.close();
    ...
    void testCashIOU ()
    {
        ...
        IOU const USD {gw["USD"]};
        {
            ...
            env.fund (CSC(1000), gw, alice, bob);
    ...
    void testCashInvalid()
    {
        ...
        env.fund (CSC(1000), gw, alice, bob, zoe);
        ...
        // Same set of failing cases for both IOU and CSC check cashing.
        ...
        failingCases (chkIdX, CSC(10));
        failingCases (chkIdU, USD(20));

        // Verify that those two checks really were cashable.
        env (check::cash (bob, chkIdU, USD(20)));
        env.close();
        env (check::cash (bob, chkIdX, check::DeliverMin (CSC(10))));
        verifyDeliveredAmount (env, CSC(10));

        // Try to cash an expired check.
        env (check::cash (bob, chkIdExp, CSC(10)), ter (tecEXPIRED));
        env.close();
    ...
    void testFix1623Enable ()
    {
        ...
        auto testEnable = [this] (FeatureBitset const& features, bool hasFields)
        {
            ...
            uint256 const chkId {getCheckIndex (alice, env.seq (alice))};
            env (check::create (alice, bob, CSC(200)));
            env.close();

            env (check::cash (bob, chkId, check::DeliverMin (CSC(100))));
