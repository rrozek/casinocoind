...
struct DepositAuth_test : public beast::unit_test::suite
{
    ...
    void testPayIOU()
    {
        // Exercise IOU payments and non-direct CSC payments to an account
        ...
        env (offer (carol, USD(100), CSC(100)));
        env.close();
        ...
        auto failedIouPayments = [this, &env, &alice, &bob, &USD] ()
        {
            ...
            PrettyAmount const bobXrpBalance {env.balance (bob, CSC)};
            PrettyAmount const bobUsdBalance {env.balance (bob, USD)};

            env (pay (alice, bob, USD (50)), ter (tecNO_PERMISSION));
            env.close();

            // Note that even though alice is paying bob in CSC, the payment
            ...
            BEAST_EXPECT (bobXrpBalance == env.balance (bob, CSC));
            BEAST_EXPECT (bobUsdBalance == env.balance (bob, USD));
        };

        //  Test when bob has an CSC balance > base reserve.
        failedIouPayments();

        // Set bob's CSC balance == base reserve.  Also demonstrate that
        // bob can make payments while his lsfDepositAuth flag is set.
        env (pay (bob, alice, USD(25)));
        env.close();

        {
            STAmount const bobPaysCSC {
                env.balance (bob, CSC) - reserve (env, 1)};
            CSCAmount const bobPaysFee {reserve (env, 1) - reserve (env, 0)};
            env (pay (bob, alice, bobPaysCSC), fee (bobPaysFee));
            env.close();
        }

        // Test when bob's CSC balance == base reserve.
        BEAST_EXPECT (env.balance (bob, CSC) == reserve (env, 0));
        BEAST_EXPECT (env.balance (bob, USD) == USD(25));
        failedIouPayments();

        // Test when bob has an CSC balance == 0.
        env (noop (bob), fee (reserve (env, 0)));
        env.close ();

        BEAST_EXPECT (env.balance (bob, CSC) == CSC (0));
        failedIouPayments();

        // Give bob enough CSC for the fee to clear the lsfDepositAuth flag.
    ...
    void testPayCSC()
    {
        // Exercise direct CSC payments to an account that has the
        ...
        env.close();
        BEAST_EXPECT (env.balance (bob, CSC) == CSC (10000) - drops(10));

        // bob has more CSC than the base reserve.  Any CSC payment should fail.
        env (pay (alice, bob, drops(1)), ter (tecNO_PERMISSION));
        env.close();
        BEAST_EXPECT (env.balance (bob, CSC) == CSC (10000) - drops(10));

        // Change bob's CSC balance to exactly the base reserve.
        {
            STAmount const bobPaysCSC {
                env.balance (bob, CSC) - reserve (env, 1)};
            CSCAmount const bobPaysFee {reserve (env, 1) - reserve (env, 0)};
        ...
        // bob has exactly the base reserve.  A small enough direct CSC
        ...
        // Take bob down to a balance of 0 CSC.
        env (noop (bob), fee (reserve (env, 0) + drops(1)));
        env.close ();
        BEAST_EXPECT (env.balance (bob, CSC) == drops(0));
        ...
        env (pay (alice, bob, reserve (env, 0) + drops(0)));
        env.close();
        BEAST_EXPECT (env.balance (bob, CSC) ==
        ...
        // Take bob back down to a zero CSC balance.
        env (noop (bob), fee (env.balance (bob, CSC)));
        env.close();
        BEAST_EXPECT (env.balance (bob, CSC) == drops(0));
        ...
        env (pay (alice, bob, drops(1)));
        env.close();
        BEAST_EXPECT (env.balance (bob, CSC) == drops(1));
    ...
    void testNoCasinocoin()
    {
            ...
            bool withDepositAuth)
        {
            ...
            TER const result = (noCasinocoinNext && noCasinocoinPrev)
                ? TER {tecPATH_DRY}
                : TER {tesSUCCESS};
...
struct DepositPreauth_test : public beast::unit_test::suite
{
    void testEnable()
    {
        ...
        Account const becky {"becky"};
        {
            ...
            env.fund (CSC (10000), alice, becky);
            env.close();
    ...
    void testInvalid()
    {
        ...
        }
        {
            ...
            Json::Value tx {deposit::auth (alice, becky)};
            tx[sfAuthorize.jsonName] = to_string (cscAccount());
        ...
        // carol gets enough CSC to (barely) meet the reserve.
    ...
    void testPayment (FeatureBitset features)
    {
...

        {
            ...
            Env env (*this, features);
            env.fund (CSC (5000), alice, becky, gw);
            env.close();
            ...
            env (offer (alice, CSC (100), USD (100), tfPassive),
            ...
            env (pay (becky, becky, USD (10)),
                path (~USD), sendmax (CSC (10)), ter (expect));
            env.close();
        }
        ...
        if (supportsPreauth)
        {
            ...
            Env env (*this, features);
            env.fund (CSC (5000), alice, becky, carol, gw);
            env.close();
            ...
            // Make CSC and IOU payments from alice to becky.  Should be fine.
            env (pay (alice, becky, CSC (100)));
            ...
            env (pay (alice, becky, CSC (100)), ter (tecNO_PERMISSION));
            ...
            env (pay (becky, alice, CSC (100)), ter (tecNO_PERMISSION));
