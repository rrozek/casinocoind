...
struct CSCEndpointStepInfo
{
    AccountID acc;
};

enum class TrustFlag {freeze, auth, noCasinocoin};
    ...
    switch(f)
    {
        ...
        case TrustFlag::auth:
            if (useHigh)
                return lsfHighAuth;
            return lsfLowAuth;
        case TrustFlag::noCasinocoin:
            if (useHigh)
                return lsfHighNoCasinocoin;
            return lsfLowNoCasinocoin;
    }
...
bool
equal(std::unique_ptr<Step> const& s1, CSCEndpointStepInfo const& cscsi)
{
    if (!s1)
        return false;
    return test::cscEndpointStepEqual(*s1, cscsi.acc);
}

bool
equal(std::unique_ptr<Step> const& s1, casinocoin::Book const& bsi)
{
    if (!s1)
        return false;
...
ape(AccountID const& a)
{
    return STPathElement(
        STPathElement::typeAccount, a, cscCurrency(), cscAccount());
};
...
{
    return STPathElement(
        STPathElement::typeIssuer, cscAccount(), cscCurrency(), account);
};
...
{
    return STPathElement(
        STPathElement::typeCurrency, cscAccount(), c, cscAccount());
};
...
public:
    ...
    valid() const
    {
        return
            ...
            (!hasAny({SB::csc, SB::existingCur, SB::prevCur}) || has(SB::cur)) &&
            // These will be duplicates
            (count({SB::csc, SB::existingCur, SB::prevCur}) <= 1) &&
...
struct ExistingElementPool
{
    ...
    std::vector<casinocoin::Currency> currencies;
        ...
        boost::optional<size_t> const& offererIndex)
    {
        ...
        // create offers to/from csc to every other ious
        for (auto const& iou : ious)
        {
            auto const owner =
                offererIndex ? accounts[*offererIndex] : iou.account;
            env(offer(owner, iou(1000), CSC(1000)), txflags(tfPassive));
            env(offer(owner, CSC(1000), iou(1000)), txflags(tfPassive));
            env.close();
        }
    }

    std::int64_t
    totalCSC(ReadView const& v, bool incRoot)
    {
        std::uint64_t totalCSC = 0;
    ...
    // Check that the balances for all accounts for all currencies & CSC are the
    // same
    bool
    checkBalances(ReadView const& v1, ReadView const& v2)
    {
        ...
        auto cscBalance = [](ReadView const& v, casinocoin::Keylet const& k) {
            auto const sle = v.read(k);
            if (!sle)
                return STAmount{};
            return (*sle)[sfBalance];
        };
        auto lineBalance = [](ReadView const& v, casinocoin::Keylet const& k) {
            auto const sle = v.read(k);
            if (!sle)
                return STAmount{};
            return (*sle)[sfBalance];
        };
        std::uint64_t totalCSC[2];
        for (auto ai1 = accounts.begin(), aie = accounts.end(); ai1 != aie;
             ++ai1)
        {
            {
                ...
                auto const b1 = cscBalance(v1, ak);
                auto const b2 = cscBalance(v2, ak);
                totalCSC[0] += b1.mantissa();
                totalCSC[1] += b2.mantissa();
                if (b1 != b2)
                    diffs.emplace_back(b1, b2, cscAccount(), *ai1);
            }
            for (auto ai2 = accounts.begin(); ai2 != aie; ++ai2)
            {
                if (ai1 >= ai2)
...
struct PayStrandAllPairs_test : public beast::unit_test::suite
{
    ...
    testAllPairs(std::initializer_list<uint256> fs)
    {
        ...
        auto callback = [&](
            ...
            std::array<PaymentSandbox, 2> sbs{
                {PaymentSandbox{env.current().get(), tapNONE},
                 PaymentSandbox{env.current().get(), tapNONE}}};
            std::array<CasinocoinCalc::Output, 2> rcOutputs;
            ...
            for (auto i = 0; i < 2; ++i)
            {
                ...
                try
                {
                    rcOutputs[i] = CasinocoinCalc::casinocoinCalculate(
                        sbs[i],
                        sendMax,
                        deliver,
                        dst,
            ...
            // check combinations of src and dst currencies (inc csc)
            // Check the results
            auto const terMatch = [&] {
                ...
                // csc followed by offer that doesn't specify both currency and
                // issuer (and currency is not csc, if specifyed)
                if (isCSC(sendMax) &&
                    !(p[0].hasCurrency() && isCSC(p[0].getCurrency())) &&
                ...
                for (size_t i = 0; i < p.size() - 1; ++i)
                {
                    ...
                    if ((tCur & STPathElement::typeCurrency) &&
                        isCSC(p[i].getCurrency()) &&
                        (tNext & STPathElement::typeAccount) &&
                        !isCSC(p[i + 1].getAccountID()))
                    {
                        return true;
                    }
                }
        ...
        for (auto const srcAmtIsCSC : {false, true})
        {
            for (auto const dstAmtIsCSC : {false, true})
            {
                for (auto const hasPrefix : {false, true})
                {
                    ...
                    STAmount const sendMax{
                        srcAmtIsCSC ? cscIssue() : Issue{eep.getAvailCurrency(),
                                                         eep.getAvailAccount()},
                        -1,  // (-1 == no limit)
                        0};

                    STAmount const deliver{
                        dstAmtIsCSC ? cscIssue() : Issue{eep.getAvailCurrency(),
                                                         eep.getAvailAccount()},
                        1,
                        0};
...
BEAST_DEFINE_TESTSUITE_MANUAL(PayStrandAllPairs, app, casinocoin);
...
struct PayStrand_test : public beast::unit_test::suite
{
    ...
    testToStrand(std::initializer_list<uint256> fs)
    {
        ...
        using B = casinocoin::Book;
        using CSCS = CSCEndpointStepInfo;
...

        {
            ...
            {
                STPath const path = STPath({ipe(USD), cpe(cscCurrency())});
                auto r = toStrand(
                    *env.current(),
                    alice,
                    alice,
...

        {
            ...
            // Path with CSC cross currency bridged payment
            test(
                env,
                EUR,
                USD.issue(),
                STPath({cpe(cscCurrency())}),
                tesSUCCESS,
                D{alice, gw, usdC},
                B{USD, CSC},
                B{CSC, EUR},
            ...
            // CSC -> CSC transaction can't include a path
            test(env, CSC, boost::none, STPath({ape(carol)}), temBAD_PATH);
            ...
            test(
                ...
                STPath({STPathElement(
                    0, cscAccount(), cscCurrency(), cscAccount())}),
                temBAD_PATH);
...

        {
            ...
            env.trust(USD(1000), alice, bob);
            env(pay(gw, alice, USD(100)));
            test(env, USD, boost::none, STPath(), terNO_CASINOCOIN);
        }
...

        {
            ...
            path.emplace_back(boost::none, cscCurrency(), boost::none);
            ...
            auto r = toStrand(
                *env.current(),
                ...
                env.app().logs().journal("Flow"));
            BEAST_EXPECT(r.first == tesSUCCESS);
            BEAST_EXPECT(equal(r.second, D{alice, gw, usdC}, B{USD.issue(), cscIssue()}, CSCS{bob}));
        }
    }
    ...
    testRIPD1373(std::initializer_list<uint256> fs)
    {
        ...
        if (hasFeature(fix1373, fs))
        {
            ...
            env(offer(bob, CSC(100), bob["USD"](100)), txflags(tfPassive));
            env(offer(gw, CSC(100), USD(100)), txflags(tfPassive));
...

        {
            ...
            // payment path: CSC -> CSC/USD -> USD/CSC
            env(pay(alice, carol, CSC(100)),
                path(~USD, ~CSC),
                txflags(tfNoCasinocoinDirect),
                ter(temBAD_SEND_CSC_PATHS));
        }
    ...
    testLoop(std::initializer_list<uint256> fs)
    {
...

        {
            ...
            // payment path: USD -> USD/CSC -> CSC/USD
            env(pay(alice, carol, USD(100)),
                sendmax(USD(100)),
                path(~CSC, ~USD),
                txflags(tfNoCasinocoinDirect),
        ...
        }
        {
            ...
            // payment path: CSC->CSC/USD->USD/EUR->USD/CNY
    ...
    testNoAccount(std::initializer_list<uint256> fs)
    {
        ...
        STPathSet pathSet;
        ::casinocoin::path::CasinocoinCalc::Input inputs;
        inputs.defaultPathsAllowed = true;
        try
        {
            PaymentSandbox sb{env.current().get(), tapNONE};
            {
                auto const r = ::casinocoin::path::CasinocoinCalc::casinocoinCalculate(
