...
class Offer_test : public beast::unit_test::suite
{
        ...
        -> jtx::PrettyAmount
    {
        ...
        return drops (dropsPerCSC<std::int64_t>::value * cscAmount - feeDrops);
    }
        ...
        jtx::Account const & acct_b, std::string const& currency)
    {
        ...
        jvParams[jss::casinocoin_state][jss::currency] = currency;
        jvParams[jss::casinocoin_state][jss::accounts] = Json::arrayValue;
        jvParams[jss::casinocoin_state][jss::accounts].append(acct_a.human());
        jvParams[jss::casinocoin_state][jss::accounts].append(acct_b.human());
...
public:
    void testRmFundedOffer (FeatureBitset features)
    {
        ...
        // be "taker gets" CSC. Old, broken would remove the first
        // "taker gets" csc offer, even though the offer is still funded and
        ...
        // "taker gets" must be CSC
        // (Different amounts so I can distinguish the offers)
        env (offer (carol, BTC (49), CSC (49)));
        env (offer (carol, BTC (51), CSC (51)));

        // Offers for the poor quality path
        // Must be two offers at the same quality
        env (offer (carol, CSC (50), USD (50)));
        env (offer (carol, CSC (50), USD (50)));

        // Offers for the good quality path
        env (offer (carol, BTC (1), USD (100)));

        PathSet paths (Path (CSC, USD), Path (USD));
        ...
        BEAST_EXPECT(!isOffer (env, carol, BTC (1), USD (100)) &&
            isOffer (env, carol, BTC (49), CSC (49)));
    }
    ...
    void testCanceledOffer (FeatureBitset features)
    {
        ...
        env (offer (alice, CSC (500), USD (100)),
            require (offers (alice, 1)));
        env.close();

        BEAST_EXPECT(isOffer (env, alice, CSC (500), USD (100)));

        // cancel the offer above and replace it with a new offer
        auto const offer2Seq = env.seq (alice);

        env (offer (alice, CSC (300), USD (100)),
             json (jss::OfferSequence, offer1Seq),
             require (offers (alice, 1)));
        env.close();

        BEAST_EXPECT(isOffer (env, alice, CSC (300), USD (100)) &&
            !isOffer (env, alice, CSC (500), USD (100)));

        // Test canceling non-existent offer.
//      auto const offer3Seq = env.seq (alice);

        env (offer (alice, CSC (400), USD (200)),
             json (jss::OfferSequence, offer1Seq),
             require (offers (alice, 2)));
        env.close();

        BEAST_EXPECT(isOffer (env, alice, CSC (300), USD (100)) &&
            isOffer (env, alice, CSC (400), USD (200)));

        // Test cancellation now with OfferCancel tx
        auto const offer4Seq = env.seq (alice);
        env (offer (alice, CSC (222), USD (111)),
            require (offers (alice, 3)));
        env.close();

        BEAST_EXPECT(isOffer (env, alice, CSC (222), USD (111)));
        {
        ...
        env.require (offers (alice, 2));
        BEAST_EXPECT( isOffer (env, alice, CSC (300), USD (100))); // offer2
        BEAST_EXPECT(!isOffer (env, alice, CSC   (5), USD   (2))); // expired
    }
    ...
    void testCSCTinyPayment (FeatureBitset features)
    {
        testcase ("CSC Tiny payments");

        // Regression test for tiny csc payments
        // In some cases, when the payment code calculates
        // the amount of csc needed as input to an csc->iou offer
        ...
        // 1) When we calculate the amount of input csc needed for an offer
        //    from csc->iou, the amount is less than 1 drop (after rounding
        ...
        for (auto withFix : {false, true})
        {
            ...
            env.fund (CSC (10000), alice, bob, carol, dan, erin, gw);
    ...
    void testEnforcenoCasinocoin (FeatureBitset features)
    {
        testcase ("Enforce No Casinocoin");
        ...
        {
            // No casinocoin with an implied account step after an offer
            Env env {*this, features};
            auto const closeTime =
                fix1449Time () +
                    100 * env.closed ()->info ().closeTimeResolution;
            ...
            env.fund (CSC (10000), alice, noCasinocoin (bob), carol, dan, gw1, gw2);
            env.trust (USD1 (1000), alice, carol, dan);
            env(trust (bob, USD1 (1000), tfSetNoCasinocoin));
            env.trust (USD2 (1000), alice, carol, dan);
            env(trust (bob, USD2 (1000), tfSetNoCasinocoin));

            env (pay (gw1, dan, USD1 (50)));
            env (pay (gw1, bob, USD1 (50)));
            env (pay (gw2, bob, USD2 (50)));

            env (offer (dan, CSC (50), USD1 (50)));

            env (pay (alice, carol, USD2 (50)), path (~USD1, bob),
                sendmax (CSC (50)), txflags (tfNoCasinocoinDirect),
                ter(tecPATH_DRY));
        }
        {
            ...
            env.fund (CSC (10000), alice, bob, carol, dan, gw1, gw2);
    ...
    testInsufficientReserve (FeatureBitset features)
    {
        ...
        // No crossing:
        {
            ...
            env (offer (alice, cscOffer, usdOffer),
        ...
        // Partial cross:
        {
            ...
            auto const cscOffer2 = CSC (500);

            env.fund (r + f + cscOffer, bob);
            env (offer (bob, usdOffer2, cscOffer2),   ter(tesSUCCESS));
            env.fund (r + f, alice);
            env (trust (alice, usdOffer),             ter(tesSUCCESS));
            env (pay (gw, alice, usdOffer),           ter(tesSUCCESS));
            env (offer (alice, cscOffer, usdOffer),   ter(tesSUCCESS));

            env.require (
                balance (alice, r - f + cscOffer2),
                balance (alice, usdOffer2),
                owners (alice, 1),
                balance (bob, r + cscOffer2),
        ...
        // buy CSC. If it fully crosses, we succeed.
        {
            ...
            env.fund (r + f + cscOffer, bob, carol);
            env (offer (bob, usdOffer2, cscOffer2),    ter(tesSUCCESS));
            env (offer (carol, usdOffer, cscOffer),    ter(tesSUCCESS));
            ...
            env.require (
                balance (alice, r - f + cscOffer),
                balance (alice, USD(0)),
                owners (alice, 1),
                balance (bob, r + cscOffer2),
                balance (bob, usdOffer2),
                owners (bob, 1),
                balance (carol, r + cscOffer2),
    ...
    testFillModes (FeatureBitset features)
    {
        ...
        auto const startBalance = CSC (1000000);
        auto const gw = Account {"gateway"};
        auto const alice = Account {"alice"};
        auto const bob = Account {"bob"};
        auto const USD = gw["USD"];
            ...
            {features - fix1578, features | fix1578})
        {
            ...
            env (offer (bob, USD (500), CSC (500)), ter(tesSUCCESS));
            env.close();
            env.require (
                owners (bob, 2),
                offers (bob, 2));
            ...
            // Order that can't be filled but will remove bob's expired offer:
            {
                ...
                env (offer (alice, CSC (1000), USD (1000)),
            ...
            env (offer (alice, CSC (500), USD (500)),
                txflags (tfFillOrKill),              ter(tesSUCCESS));

            env.require (
                balance (alice, startBalance - (f * 3) + CSC (500)),
                balance (alice, USD (500)),
                owners (alice, 1),
                offers (alice, 0),
                balance (bob, startBalance - (f * 2) - CSC (500)),
                balance (bob, USD (500)),
                owners (bob, 1),
                offers (bob, 0));
        }
        ...
        // and add nothing on the books:
        {
            ...
            // Partially cross:
            env (offer (bob, USD (50), CSC (50)),            ter(tesSUCCESS));
            env (offer (alice, CSC (1000), USD (1000)),
                txflags (tfImmediateOrCancel),              ter(tesSUCCESS));

            env.require (
                balance (alice, startBalance - f - f - f + CSC (50)),
                balance (alice, USD (950)),
                owners (alice, 1),
                offers (alice, 0),
                balance (bob, startBalance - f - CSC (50)),
                balance (bob, USD (50)),
                owners (bob, 1),
                offers (bob, 0));
            ...
            // Fully cross:
            env (offer (bob, USD (50), CSC (50)),            ter(tesSUCCESS));
            env (offer (alice, CSC (50), USD (50)),
                txflags (tfImmediateOrCancel),               ter(tesSUCCESS));

            env.require (
                balance (alice, startBalance - f - f - f - f + CSC (100)),
                balance (alice, USD (900)),
                owners (alice, 1),
                offers (alice, 0),
                balance (bob, startBalance - f - f - CSC (100)),
                balance (bob, USD (100)),
                owners (bob, 1),
                offers (bob, 0));
        }
        ...
        // tfPassive -- place the offer without crossing it.
        {
            ...
            env (offer (alice, USD(1000), CSC(2000)));
            env.close();
            ...
            for (auto offerPtr : aliceOffers)
            {
                ...
                BEAST_EXPECT (offer[sfTakerGets] == CSC (2000));
            ...
            env (offer (bob, CSC(2000), USD(1000), tfPassive));
            env.close();
            ...
            for (auto offerPtr : bobOffers)
            {
                ...
                BEAST_EXPECT (offer[sfTakerPays] == CSC (2000));
            }

            // It should be possible for gw to cross both of those offers.
            env (offer (gw, CSC(2000), USD(1000)));
            env.close();
            env.require (offers (alice, 0));
            env.require (offers (gw, 0));
            env.require (offers (bob, 1));

            env (offer (gw, USD(1000), CSC(2000)));
            env.close();
        ...
        // tfPassive -- cross only offers of better quality.
        {
            ...
            env (pay (gw, "bob", USD(1000)));
            env (offer ("alice", USD(500), CSC(1001)));
            env.close();

            env (offer ("alice", USD(500), CSC(1000)));
            env.close();
            ...
            for (auto offerPtr : bobOffers)
            {
                ...
                BEAST_EXPECT (offer[sfTakerPays] == CSC (999));
            }
        }
    }
    ...
    testMalformed(FeatureBitset features)
    {
        ...
        env (offer (alice, USD (1000), CSC (1000)),
        ...
        {
            // Alice tries an CSC to CSC order:
            env (offer (alice, CSC (1000), CSC (1000)),   ter(temBAD_OFFER));
            env.require (
                owners (alice, 0),
                offers (alice, 0));
        ...
        {
            env (offer (alice, -USD (1000), CSC (1000)),  ter(temBAD_OFFER));
            env.require (
                owners (alice, 1),
                offers (alice, 0));
        ...
        // Use CSC as a currency code
        {
            auto const BAD = IOU(gw, badCurrency());

            env (offer (alice, CSC (1000), BAD (1000)),   ter(temBAD_CURRENCY));
            env.require (
                owners (alice, 1),
                offers (alice, 0));
        }
    ...
    testExpiration(FeatureBitset features)
    {
        ...
        env (offer (bob, usdOffer, cscOffer),     ter(tesSUCCESS));
        env.require (
            balance (alice, startBalance - f - f - f),
            balance (alice, usdOffer),
            offers (alice, 0),
    ...
    testUnfundedCross(FeatureBitset features)
    {
        ...
        env (offer ("alice", usdOffer, cscOffer),     ter(tecUNFUNDED_OFFER));
        env.require (
        ...
        env (offer ("bob", usdOffer, cscOffer),       ter(tecUNFUNDED_OFFER));
        env.require (
        ...
        env.fund (reserve (env, 0) + f + CSC(1), "carol");
        env (offer ("carol", usdOffer, cscOffer), ter(tecINSUF_RESERVE_OFFER));
        env.require (
            balance ("carol", reserve (env, 0) + CSC(1)),
            owners ("carol", 0));

        // Account has enough for the reserve plus one
        // offer, and the fee.
        env.fund (reserve (env, 1) + f, "dan");
        env (offer ("dan", usdOffer, cscOffer),       ter(tesSUCCESS));
        env.require (
        ...
        env.fund (reserve (env, 1) + f + cscOffer, "eve");
        env (offer ("eve", usdOffer, cscOffer),       ter(tesSUCCESS));
        env.require (
            balance ("eve", reserve (env, 1) + cscOffer),
            owners ("eve", 1));
    }
    ...
    testSelfCross(bool use_partner, FeatureBitset features)
    {
        ...
        // old and the new behavior.
        {
            ...
            for (auto const& offerPtr : acctOffers)
            {
                ...
                BEAST_EXPECT (offer[sfTakerPays] == CSC (0));
            }
        }
    ...
    testNegativeBalance(FeatureBitset features)
    {
        ...
        env (offer (alice, USD (50), CSC (150000)));
        ...
        // small_amount and transfer no CSC.  The new offer crossing transfers
        ...
        BEAST_EXPECT(jrr[jss::node][sfBalance.fieldName][jss::value] == "50");
        BEAST_EXPECT(env.balance (alice, cscIssue()) ==
        ...
        BEAST_EXPECT(env.balance (bob, cscIssue()) ==
            bob_initial_balance -
    ...
    void
    testOfferCrossWithCSC(bool reverse_order, FeatureBitset features)
    {
        testcase (std::string("Offer Crossing with CSC, ") +
        ...
        env.fund (CSC (10000), gw, alice, bob);
        ...
        if (reverse_order)
            env (offer (bob, USD (1), CSC (4000)));

        env (offer (alice, CSC (150000), USD (50)));
        ...
        BEAST_EXPECT(
            ...
            std::to_string(
                CSC (10000).value ().mantissa () -
                CSC (reverse_order ? 4000 : 3000).value ().mantissa () -
    ...
    testOfferCrossWithLimitOverride(FeatureBitset features)
    {
        ...
        env.fund (CSC (100000), gw, alice, bob);
        ...
        BEAST_EXPECT(
            jrr[jss::node][sfBalance.fieldName] ==
            std::to_string(
                CSC (100000).value ().mantissa () -
                CSC (3000).value ().mantissa () -
    ...
    testOfferAcceptThenCancel(FeatureBitset features)
    {
        ...
        env (offer (env.master, CSC (500), USD (100)));
        env.close ();
    ...
    testCurrencyConversionEntire(FeatureBitset features)
    {
        ...
        BEAST_EXPECT(
            jro[jss::node][jss::TakerGets] == CSC (500).value ().getText ());
        BEAST_EXPECT(
            jro[jss::node][jss::TakerPays] == USD (100).value ().getJson (0));

        env (pay (alice, alice, CSC (500)), sendmax (USD (100)));
    ...
    testCurrencyConversionInParts(FeatureBitset features)
    {
        ...
        env (pay (alice, alice, CSC (200)), sendmax (USD (100)));

        // The previous payment reduced the remaining offer amount by 200 CSC
        auto jro = ledgerEntryOffer (env, bob, bobOfferSeq);
        BEAST_EXPECT(
            jro[jss::node][jss::TakerGets] == CSC (300).value ().getText ());
        BEAST_EXPECT(
        ...
        // alice now has 200 more CSC from the payment
        jrr = ledgerEntryRoot (env, alice);
        BEAST_EXPECT(
            jrr[jss::node][sfBalance.fieldName] ==
            std::to_string(
                CSC (10000).value ().mantissa () +
                CSC (200).value ().mantissa () -
                env.current ()->fees ().base * 2)
        );
        ...
        // Alice converts USD to CSC which should fail
        // due to PartialPayment.
        env (pay (alice, alice, CSC (600)), sendmax (USD (100)),
            ter(tecPATH_PARTIAL));

        // Alice converts USD to CSC, should succeed because
        ...
        // only 300 CSC should be have been payed since that's all
    ...
    void
    testCrossCurrencyStartCSC(FeatureBitset features)
    {
        testcase ("Cross Currency Payment: Start with CSC");
        ...
        env (offer (carol, CSC (500), USD (50)));

        env (pay (alice, bob, USD (25)), sendmax (CSC (333)));
        ...
        BEAST_EXPECT(
            jro[jss::node][jss::TakerPays] == CSC (250).value ().getText ());
    }

    void
    testCrossCurrencyEndCSC(FeatureBitset features)
    {
        testcase ("Cross Currency Payment: End with CSC");
        ...
        auto const carolOfferSeq = env.seq (carol);
        env (offer (carol, USD (50), CSC (500)));

        env (pay (alice, bob, CSC (250)), sendmax (USD (333)));
        ...
        BEAST_EXPECT(
            ...
            std::to_string(
                CSC (10000).value ().mantissa () +
                CSC (250).value ().mantissa ())
        );

        auto jro = ledgerEntryOffer (env, carol, carolOfferSeq);
        BEAST_EXPECT(
            jro[jss::node][jss::TakerGets] == CSC (250).value ().getText ());
        BEAST_EXPECT(
    ...
    testCrossCurrencyBridged(FeatureBitset features)
    {
        ...
        env.fund (CSC (10000), gw1, gw2, alice, bob, carol, dan);
        ...
        env (offer (dan, CSC (500), EUR (50)));

        Json::Value jtp {Json::arrayValue};
        jtp[0u][0u][jss::currency] = "CSC";
        ...
        auto jro = ledgerEntryOffer (env, carol, carolOfferSeq);
        BEAST_EXPECT(
            jro[jss::node][jss::TakerGets] == CSC (200).value ().getText ());
        BEAST_EXPECT(
        ...
        BEAST_EXPECT(
            jro[jss::node][jss::TakerPays] == CSC (200).value ().getText ());
    }
    ...
    testBridgedSecondLegDry(FeatureBitset features)
    {
        ...
        env (offer (alice, EUR(2), CSC(1)));
        env (offer (alice, EUR(2), CSC(1)));

        env (offer (alice, CSC(1), USD(4)));
        env (offer (carol, CSC(1), USD(3)));
        env.close();
    ...
    testOfferFeesConsumeFunds(FeatureBitset features)
    {
        ...
        //  1 for payment          == 4
        auto const starting_csc = CSC (100) +
            env.current ()->fees ().accountReserve (3) +
            env.current ()->fees ().base * 4;

        env.fund (starting_csc, gw1, gw2, gw3, alice, bob);
    ...
    testSellFlagBasic(FeatureBitset features)
    {
        ...
        env.fund (starting_csc, gw, alice, bob);
        ...
        env (offer (bob, CSC (200), USD (200)), json(jss::Flags, tfSell));
        // Alice has 350 + fees - a reserve of 50 = 250 reserve = 100 available.
        // Alice has 350 + fees - a reserve of 50 = 250 reserve = 100 available.
        // Ask for more than available to prove reserve works.
        env (offer (alice, USD (200), CSC (200)), json(jss::Flags, tfSell));
    ...
    testSellFlagExceedLimit(FeatureBitset features)
    {
        ...
        env (offer (bob, CSC (100), USD (200)));
        // Alice has 350 fees - a reserve of 50 = 250 reserve = 100 available.
        // Ask for more than available to prove reserve works.
        // Taker pays 100 USD for 100 CSC.
        // Selling CSC.
        // Will sell all 100 CSC and get more USD than asked for.
        env (offer (alice, USD (100), CSC (100)), json(jss::Flags, tfSell));
    ...
    testGatewayCrossCurrency(FeatureBitset features)
    {
        ...
        auto const starting_csc = CSC (100.1) +
            env.current ()->fees ().accountReserve (1) +
            env.current ()->fees ().base * 2;
        ...
        if (wsc->version() == 2)
        {
            ...
            BEAST_EXPECT(jrr.isMember(jss::casinocoinrpc) && jrr[jss::casinocoinrpc] == "2.0");
    ...
    void testPartialCross (FeatureBitset features)
    {
        ...
        env.fund (CSC(10000000), gw);

        // The fee that's charged for transactions
        auto const f = env.current ()->fees ().base;

        // To keep things simple all offers are 1 : 1 for CSC : USD.
        enum preTrustType {noPreTrust, gwPreTrust, acctPreTrust};
        struct TestData
        {
            std::string account;       // Account operated on
            STAmount fundCsc;          // Account funded with
            int bookAmount;            // USD -> CSC offer on the books
            preTrustType preTrust;     // If true, pre-establish trust line
            int offerAmount;           // Account offers this much CSC -> USD
            TER tec;                   // Returned tec code
            STAmount spentCsc;         // Amount removed from fundCsc
        ...
        {
//acct                             fundCsc  bookAmt      preTrust  offerAmt                     tec       spentCsc    balanceUSD  offers  owners
...
{"ova", CSC( 999) + reserve (env, 0) + 1*f,    1000,   noPreTrust,     1000,             tesSUCCESS, CSC( 999) + f, USD(    999),      0,     1},
{"pam", CSC( 999) + reserve (env, 1) + 1*f,    1000,   noPreTrust,     1000,             tesSUCCESS, CSC(1000) + f, USD(   1000),      0,     1},
{"rae", CSC( 999) + reserve (env, 2) + 1*f,    1000,   noPreTrust,     1000,             tesSUCCESS, CSC(1000) + f, USD(   1000),      0,     1},
{"sue", CSC(1000) + reserve (env, 2) + 1*f,       0,   noPreTrust,     1000,             tesSUCCESS,             f, USD(      0),      1,     1},
...
{"hal",             reserve (env, 1) + 1*f,       1,   gwPreTrust,     1000,             tesSUCCESS, CSC(   1) + f, USD(      1),      1,     1},
...
{"xan",             reserve (env, 2) + 1*f,       1, acctPreTrust,     1000,             tesSUCCESS, CSC( 1) + 2*f, USD(      1),      1,     2},
};

        for (auto const& t : tests)
        {
            auto const acct = Account(t.account);
            env.fund (t.fundCsc, acct);
            env.close();
            ...
            if (book)
                env (offer (gw, CSC (book), USD (book)));
            env.close();
            ...
            // to be enough CSC reserve for acct to create the trust line.
            ...
            env (offer (acct, USD (acctOffer), CSC (acctOffer)), ter (t.tec));
            env.close();
            std::uint32_t const acctOfferSeq = env.seq (acct) - 1;

            BEAST_EXPECT (env.balance (acct, USD.issue()) == t.balanceUsd);
            BEAST_EXPECT (
                env.balance (acct, cscIssue()) == t.fundCsc - t.spentCsc);
            ...
            if (acctOffers.size() && t.offers)
            {
                ...
                BEAST_EXPECT (acctOffer[sfTakerGets] == CSC (leftover));
    ...
    void
    testCSCDirectCross (FeatureBitset features)
    {
        testcase ("CSC Direct Crossing");
        ...
        // The scenario:
        //   o alice has USD but wants CSC.
        //   o bob has CSC but wants USD.
        auto const alicesCSC = env.balance (alice);
        auto const bobsCSC = env.balance (bob);

        env (offer (alice, cscOffer, usdOffer));
        env.close();
        env (offer (bob, usdOffer, cscOffer));

        env.close();
        env.require (
            balance (alice, USD(0)),
            balance (bob, usdOffer),
            balance (alice, alicesCSC + cscOffer - fee),
            balance (bob,   bobsCSC   - cscOffer - fee),
            offers (alice, 0),
            offers (bob, 0));
        ...
        env (offer (alice, USD(999), CSC(999)));
        env (offer (bob, cscOffer, usdOffer));
        ...
        verifyDefaultTrustline (env, bob, USD(1));
        {
            ...
            BEAST_EXPECT (bobsOffer[sfTakerPays] == CSC (1));
        }
   }
    ...
    testBridgedCross (FeatureBitset features)
    {
        ...
        env.fund (CSC(1000000), gw, alice, bob, carol);
        env.close();
        ...
        env (offer (alice, CSC(1000), usdOffer));
        env (offer (bob, eurOffer, CSC(1000)));
        auto const bobCscBalance = env.balance (bob);
        env.close();
        ...
        env.require (
            ...
            balance (bob, bobCscBalance - CSC(400)),
            offers (carol, 0));
        ...
        verifyDefaultTrustline (env, carol, USD(400));
        {
            ...
            BEAST_EXPECT (alicesOffer[sfTakerPays] == CSC (600));
        }
        {
            auto const bobsOffers = offersOnAccount (env, bob);
            BEAST_EXPECT (bobsOffers.size() == 1);
            ...
            BEAST_EXPECT (bobsOffer[sfLedgerEntryType] == ltOFFER);
            BEAST_EXPECT (bobsOffer[sfTakerGets] == CSC (600));
        ...
        env.require (
            ...
            balance (bob, bobCscBalance - CSC(1000)),
            offers (bob, 0),
            offers (carol, 0));
        ...
        if (alicesOffers.size() != 0)
        {
            ...
            BEAST_EXPECT (alicesOffer[sfTakerPays] == CSC (0));
        }
    }
    ...
    testSellOffer (FeatureBitset features)
    {
        ...
        {
            std::string account;       // Account operated on
            STAmount fundCsc;          // CSC acct funded with
            ...
            TestData (
                ...
                STAmount const& fundCsc_,       // CSC acct funded with
                ...
                STAmount const& spentCsc_,      // Amount removed from fundCsc
            ...
            TestData (
                ...
                int offers_,                    // Offers on acct
                int owners_)                    // Owners on acct
                : TestData (std::move(account_), fundCsc_, fundUSD_, gwGets_,
                  gwPays_, acctGets_, acctPays_, tec_, spentCsc_, finalUsd_,
...
{"ova",           reserve (env, 1) + 2*f, USD(10), USD(10), CSC(10),  CSC(10),  USD(10),             tesSUCCESS, CSC(-10) + (2*f),  USD( 0),      0,     1},
{"pam",           reserve (env, 1) + 2*f, USD(10), USD(10), CSC(20),  CSC(10),  USD(10),             tesSUCCESS, CSC(-20) + (2*f),  USD( 0),      0,     1},
{"qui",           reserve (env, 1) + 2*f, USD(10), USD(20), CSC(40),  CSC(10),  USD(10),             tesSUCCESS, CSC(-20) + (2*f),  USD( 0),      0,     1},
{"rae",           reserve (env, 2) + 2*f, USD(10), USD( 5), CSC( 5),  CSC(10),  USD(10),             tesSUCCESS, CSC( -5) + (2*f),  USD( 5),      1,     2,     USD( 5),  CSC( 5)},
{"sue",           reserve (env, 2) + 2*f, USD(10), USD( 5), CSC(10),  CSC(10),  USD(10),             tesSUCCESS, CSC(-10) + (2*f),  USD( 5),      1,     2,     USD( 5),  CSC( 5)},
};
    ...
    testSellWithFillOrKill (FeatureBitset features)
    {
        ...
        env.fund (CSC(10000000), gw, alice, bob);
        ...
        }
        {
            ...
            env (offer(alice, USD(20), CSC(2000), tfSell | tfFillOrKill));
            env.close();
        ...
        }
        {
            ...
            env (offer(bob, CSC(2000), USD(20)));
            env.close();
            env (offer(alice, USD(10), CSC(1500), tfSell | tfFillOrKill));
            env.close();
        ...
        }
        {
            ...
            // We're using bob's left-over offer for CSC(500), USD(5)
            env (offer(alice, USD(1), CSC(501),
                tfSell | tfFillOrKill), ter (killedCode));
            env.close();
            env.require (balance (alice, USD(35)));
            env.require (offers (alice, 0));
        ...
        }
        {
            ...
            // We're using bob's left-over offer for CSC(500), USD(5)
            env (offer(alice, USD(1), CSC(500), tfSell | tfFillOrKill));
            env.close();
    ...
    testTransferRateOffer (FeatureBitset features)
    {
        ...
        env(rate(gw1, 1.25));
        {
            ...
            auto const bob = Account("bob");
            env.fund (CSC(100) + reserve(env, 2) + (fee*2), ann, bob);
            env.close();
            ...
            // bob offers to sell USD(100) for CSC.  alice takes bob's offer.
            ...
            env (offer (bob, CSC(1), USD(100)));
            env.close();

            env (offer (ann, USD(100), CSC(1)));
            env.close();

            env.require (balance (ann, USD(100)));
            env.require (balance (ann, CSC( 99) + reserve(env, 2)));
            env.require (offers (ann, 0));

            env.require (balance (bob, USD(  0)));
            env.require (balance (bob, CSC(101) + reserve(env, 2)));
            env.require (offers (bob, 0));
        }
        {
            // Reverse the order, so the offer in the books is to sell CSC
            // in return for USD.  Gateway rate should still apply identically.
            auto const che = Account("che");
            auto const deb = Account("deb");
            env.fund (CSC(100) + reserve(env, 2) + (fee*2), che, deb);
            env.close();
            ...
            env (offer (che, USD(100), CSC(1)));
            env.close();

            env (offer (deb, CSC(1), USD(100)));
            env.close();

            env.require (balance (che, USD(100)));
            env.require (balance (che, CSC( 99) + reserve(env, 2)));
            env.require (offers (che, 0));

            env.require (balance (deb, USD(  0)));
            env.require (balance (deb, CSC(101) + reserve(env, 2)));
        ...
        }
        {
            ...
            env.fund (CSC(20000) + fee*2, eve, fyn);
            env.close();
            ...
            env (offer (eve, USD(10), CSC(4000)));
            env.close();
            std::uint32_t const eveOfferSeq = env.seq (eve) - 1;

            env (offer (fyn, CSC(2000), USD(5)));
            env.close();

            env.require (balance (eve, USD(105)));
            env.require (balance (eve, CSC(18000)));
            ...
            if (evesOffers.size() != 0)
            {
                ...
                BEAST_EXPECT (evesOffer[sfTakerGets] == CSC (2000));
            ...
            env.require (balance (fyn, CSC(22000)));
        ...
        env.fund (CSC(100000), gw2);
        env.close();

        env(rate(gw2, 1.5));
        {
            // Remove CSC from the equation.  Give the two currencies two
        ...
        }
        {
            ...
            env.fund (CSC(2) + reserve(env, 3) + (fee*3), ova, pat, qae);
            env.close();

            //   o ova has USD but wants XPR.
            //   o pat has CSC but wants EUR.
            ...
            env (offer (ova, CSC(2), USD(100)));
            env (offer (pat, EUR(100), CSC(2)));
            env.close();
            ...
            env.require (balance (ova, CSC(4) + reserve(env, 3)));
    ...
    testSelfCrossOffer1 (FeatureBitset features)
    {
        ...
        for (auto const& offerPtr : gwOffers)
        {
            auto const& offer = *offerPtr;
            BEAST_EXPECT (offer[sfLedgerEntryType] == ltOFFER);
            BEAST_EXPECT (offer[sfTakerGets] == CSC (600));
        ...
        for (auto const& offerPtr : gwOffers)
        {
            ...
            BEAST_EXPECT (offer[sfTakerPays] == CSC (1000));
        }
    }
    ...
    testSelfCrossOffer2 (FeatureBitset features)
    {
        ...
        TestData const tests[]
        {
// acct               fundCSC    fundUSD    fundEUR           firstOfferTec          secondOfferTec
...
{"flo", reserve(env, 3) +   0, USD(1000), EUR(1000), tecINSUF_RESERVE_OFFER, tecINSUF_RESERVE_OFFER},
    ...
    testSelfIssueOffer (FeatureBitset features)
    {
        ...
        env.fund(CSC(50000) + f, alice, bob);
        env.close();

        env(offer(alice, USD(5000), CSC(50000)));
        env.close();

        // This offer should take alice's offer up to Alice's reserve.
        env(offer(bob, CSC(50000), USD(5000)));
        env.close();
    ...
    void testSelfCrossLowQualityOffer (FeatureBitset features)
    {
        ...
        // for more CSC than the offering account held.  This unit test
    ...
    void testTinyOffer (FeatureBitset features)
    {
        ...
        auto const CNY = gw["CNY"];
        auto const fee = env.current()->fees().base;
        auto const startCscBalance = drops (400000000000) + (fee * 2);

        env.fund (startCscBalance, gw, alice, bob);
        env.close();
        ...
        env.require (balance (alice, startCscBalance - fee - drops(1)));
        env.require (balance (bob, bobsCnyStartBalance - alicesCnyOffer));
        env.require (balance (bob, startCscBalance - (fee * 2) + drops(1)));
    }
    ...
    void testSelfPayXferFeeOffer (FeatureBitset features)
    {
        ...
        //
        //  2. alice makes an offer to buy CSC and sell USD.
        //  3. bob makes an offer to buy BTC and sell CSC.
        //
        ...
        //  o bob' offer which takes alice's BTC and sells CSC to...
        //  o alice's first offer which takes bob's CSC and sells USD to...
        ...
        auto const BTC = gw["BTC"];
        auto const USD = gw["USD"];
        auto const startCscBalance = CSC (4000000);

        env.fund (startCscBalance, gw);
        env.close();
        ...
        struct Actor
        {
            ...
            PrettyAmount csc;  // final expected after crossing
...
// { 0, 0, 0, BTC( 5), { {"flo", 0, drops(399999995000000), BTC( 5.0), USD(2000)}                                                      } }  // no xfer fee
        ...
        for (auto const& t : tests)
        {
            ...
            {
                env.fund (CSC (4000000), actor.acct);
                env.close();
            ...
            env (offer (leg0, BTC(10),     CSC(100000), tfPassive));
            env.close();
            std::uint32_t const leg0OfferSeq = env.seq (leg0) - 1;

            env (offer (leg1, CSC(100000), USD(1000),   tfPassive));
            env.close();
...
{ 0, 0, 0, BTC( 5), { {"hye", 2, drops(399999995000000), BTC(5), USD(2000)}                                                      } }  // no xfer fee
    ...
    void testRequireAuth (FeatureBitset features)
    {
        ...
        env.fund (CSC(400000), gw, alice, bob);
        env.close();
        ...
        env (offer (alice, gwUSD(40), CSC(4000)));
        env.close();
        ...
        env (offer (bob, CSC(4000), gwUSD(40)));
        env.close();
    ...
    void testMissingAuth (FeatureBitset features)
    {
        ...
        // untouched.
        env (offer (alice, gwUSD(40), CSC(4000)), ter(tecNO_LINE));
        env.close();
        ...
        env (offer (alice, gwUSD(40), CSC(4000)), ter(tecNO_AUTH));
        env.close();
    ...
    {
        testcase("CasinocoinConnect Smoketest payment flow");
        using namespace jtx;
        ...
        // This test mimics the payment flow used in the Casinocoin Connect
        ...
        env.fund (CSC(100000), hotUS, coldUS, hotEU, coldEU, mm);
        env.close();

        // Cold wallets require trust but will casinocoin by default
        for (auto const& cold : {coldUS, coldEU})
        {
            env(fset (cold, asfRequireAuth));
            env(fset (cold, asfDefaultCasinocoin));
        }
        env.close();

        // Each hot wallet trusts the related cold wallet for a large amount
        env (trust(hotUS, USD(10000000)), txflags (tfSetNoCasinocoin));
        env (trust(hotEU, EUR(10000000)), txflags (tfSetNoCasinocoin));
        // Market maker trusts both cold wallets for a large amount
        env (trust(mm, USD(10000000)), txflags (tfSetNoCasinocoin));
        env (trust(mm, EUR(10000000)), txflags (tfSetNoCasinocoin));
        env.close();
        ...
        // There should be a path available from hot US to cold EUR
        {
            ...
            Json::Value const jrr {env.rpc(
                "json", "casinocoin_path_find", to_string(jvParams))[jss::result]};
    ...
    void testSelfAuth (FeatureBitset features)
    {
        ...
        env.fund (CSC(400000), gw, alice);
        env.close();

        // Test that gw can create an offer to buy gw's currency.
        env (offer (gw, gwUSD(40), CSC(4000)));
        env.close();
        std::uint32_t const gwOfferSeq = env.seq (gw) - 1;
        env.require (offers (gw, 1));
        ...
        env (offer (alice, CSC(4000), gwUSD(40)));
        env.close();
    ...
    void testAll(FeatureBitset features)
    {
        ...
        testCSCTinyPayment(features);
        testEnforcenoCasinocoin(features);
        ...
        testOfferCrossWithCSC(true, features);
        testOfferCrossWithCSC(false, features);
        ...
        testCrossCurrencyStartCSC(features);
        testCrossCurrencyEndCSC(features);
...
BEAST_DEFINE_TESTSUITE_PRIO (Offer, tx, casinocoin, 4);
BEAST_DEFINE_TESTSUITE_MANUAL_PRIO (Offer_manual, tx, casinocoin, 20);
