...
public:
    ...
    testStepLimit(FeatureBitset features)
    {
        ...
        env.fund(CSC(100000000), gw, "alice", "bob", "carol", "dan");
        env.trust(USD(1), "bob");
        env(pay(gw, "bob", USD(1)));
        env.trust(USD(1), "dan");
        env(pay(gw, "dan", USD(1)));
        n_offers (env, 2000, "bob", CSC(1), USD(1));
        n_offers (env, 1, "dan", CSC(1), USD(1));

        // Alice offers to buy 1000 CSC for 1000 USD. She takes Bob's first
        ...
        // Carol offers to buy 1000 CSC for 1000 USD. She removes Bob's next
        // 1000 offers as unfunded and hits the step limit.
        env(offer("carol", USD(1000), CSC(1000)));
    ...
    testStepAndCrossingLimit(FeatureBitset features)
    {
        ...
        env.fund(CSC(100000000), gw, "alice", "bob", "carol", "dan", "evita");
        ...
        n_offers (env, 400, "alice", CSC(1), USD(1));
        n_offers (env, carolsOfferCount, "carol", CSC(1), USD(1));
        n_offers (env, evitasOfferCount, "evita", CSC(1), USD(1));

        // Bob offers to buy 1000 CSC for 1000 USD. He takes all 400 USD from
        // Alice's offers, 1 USD from Carol's and then removes 599 of Carol's
        // offers as unfunded, before hitting the step limit.
        env(offer("bob", USD(1000), CSC(1000)));
        ...
        // Dan offers to buy maxConsumed + 50 CSC USD. He removes all of
        // Carol's remaining offers as unfunded, then takes
        // (maxConsumed - 100) USD from Evita's, hitting the crossing limit.
        env(offer("dan", USD(maxConsumed + 50), CSC(maxConsumed + 50)));
    ...
    void testAutoBridgedLimitsTaker (FeatureBitset features)
    {
        ...
        n_offers (env,  302, "alice", EUR(2), CSC(1));
        n_offers (env,  300, "alice", CSC(1), USD(4));
        n_offers (env,  497, "carol", CSC(1), USD(3));
        ...
        //  2. He spends another 2 EUR taking one of Alice's EUR->CSC and
        //     one of Carol's CSC-USD offers.  He gets 3 USD for that.
    ...
    testAutoBridgedLimitsFlowCross(FeatureBitset features)
    {
        ...
        // strand dry until the liquidity is actually used)
        {
            ...
            // best quality
            n_offers(env, 2000, alice, EUR(2), CSC(1));
            n_offers(env, 300, alice, CSC(1), USD(4));
            n_offers(
                env, 801, carol, CSC(1), USD(3));  // only one offer is funded
            n_offers(env, 1000, alice, CSC(1), USD(3));
            ...
            //     c. 199 of alice's CSC(1) to USD(3) offers are consumed.
            ...
            //     CSC(1) to USD(3) offers available. Bob has spent 400 EUR and
    ...
    testOfferOverflow (FeatureBitset features)
    {
        ...
        env.fund(CSC(100000000), gw, alice, bob);
        ...
        n_offers(env, 998, alice, CSC(1.00), USD(1));
        n_offers(env, 998, alice, CSC(0.99), USD(1));
        n_offers(env, 998, alice, CSC(0.98), USD(1));
        n_offers(env, 998, alice, CSC(0.97), USD(1));
        n_offers(env, 998, alice, CSC(0.96), USD(1));
        n_offers(env, 998, alice, CSC(0.95), USD(1));

        bool const withFlowCross = features[featureFlowCross];
        env(offer(bob, USD(8000), CSC(8000)), ter(withFlowCross ? TER{tecOVERSIZE} : tesSUCCESS));
        env.close();
...
BEAST_DEFINE_TESTSUITE_MANUAL_PRIO(CrossingLimits,tx,casinocoin,10);
