...
class Discrepancy_test : public beast::unit_test::suite
{
    ...
    testXRPDiscrepancy (FeatureBitset features)
    {
        ...
        env.fund(CSC(2000), A1);
        env.fund(CSC(1000), A2, A6, A7);
        env.fund(CSC(5000), A3);
        env.fund(CSC(1000000), A4);
        env.fund(CSC(600000), A5);
        env.close();
        ...
        env(offer(A4, CSC(49147), A2["JPY"](34501)));
        env(offer(A5, A3["CNY"](3150), CSC(80086)));
        env(offer(A7, CSC(1233), A6["CNY"](25)));
        env.close();

        test::PathSet payPaths {
            test::Path {A2["JPY"], A2},
            test::Path {CSC, A2["JPY"], A2},
            test::Path {A6, CSC, A2["JPY"], A2} };
