...
auto IPE(Issue const& iss)
{
    return STPathElement (
        ...
        cscAccount (), iss.currency, iss.account);
};
...
public:
    ...
    path_find_consume_all()
    {
...

        {
            ...
            env(offer("carol", CSC(100), USD(100)));
            ...
            std::tie(st, sa, da) = find_paths(env,
                "alice", "bob", Account("bob")["AUD"](-1),
                    boost::optional<STAmount>(CSC(100000000)));
            BEAST_EXPECT(st.empty());
            std::tie(st, sa, da) = find_paths(env,
                "alice", "bob", Account("bob")["USD"](-1),
                    boost::optional<STAmount>(CSC(100000000)));
            BEAST_EXPECT(sa == CSC(100));
    ...
    alternative_path_consume_both()
    {
        ...
        env.fund(CSC(10000), "alice", "bob", gw, gw2);
        env.trust(USD(600), "alice");
    ...
    alternative_paths_limit_returned_paths_to_best_quality()
    {
        ...
        auto const gw2 = Account("gateway2");
        auto const gw2_USD = gw2["USD"];
        env.fund(CSC(10000), "alice", "bob", "carol", "dan", gw, gw2);
        env(rate("carol", 1.1));
    ...
    issues_path_negative_ripple_client_issue_23_larger()
    {
        ...
        using namespace jtx;
        Env env(*this);
        env.fund(CSC(10000), "alice", "bob", "carol", "dan", "edward");
    ...
    // carol holds gateway AUD, sells gateway AUD for CSC
    // bob will hold gateway AUD
    // alice pays bob gateway AUD using CSC
    void
    via_offers_via_gateway()
    {
        ...
        env(offer("carol", CSC(50), AUD(50)));
        env(pay("alice", "bob", AUD(10)), sendmax(CSC(100)), paths(CSC));
    ...
    void path_find_01()
    {
        testcase("Path Find: CSC -> CSC and CSC -> IOU");
        using namespace jtx;
        Env env(*this);
        Account A1 {"A1"};
        Account A2 {"A2"};
        ...
        env.fund(CSC(100000), A1);
        env.fund(CSC(10000), A2);
        env.fund(CSC(1000), A3, G1, G2, G3, M1);
        env.close();
        ...
        env(offer(M1, CSC(10000), G3["ABC"](1000)));

        STPathSet st;
        STAmount sa, da;

        {
            auto const& send_amt = CSC(10);
    ...
    void path_find_02()
    {
        testcase("Path Find: non-CSC -> CSC");
        using namespace jtx;
        Env env(*this);
        Account A1 {"A1"};
        Account A2 {"A2"};
        ...
        env.fund(CSC(1000), A1, A2, G3);
        env.fund(CSC(11000), M1);
        env.close();
        ...
        env(offer(M1, G3["ABC"](1000), CSC(10000)));
        ...
        BEAST_EXPECT(same(st, stpath(G3, IPE(cscIssue()))));
    }
    ...
    void path_find_03()
    {
        ...
        env.fund(CSC(4291.430036), MONEY_MAKER_1);
        env.fund(CSC(106839.375770), MONEY_MAKER_2);
        env.fund(CSC(1240.997150), A1);
        env.fund(CSC(14115.046893), A2);
        env.fund(CSC(512087.883181), A3);
        env.close();
        ...
        env(offer(MONEY_MAKER_2, CSC(1), GATEWAY_DST["CNY"](1)));
        env(offer(MONEY_MAKER_2, GATEWAY_DST["CNY"](1), CSC(1)));
        env(offer(MONEY_MAKER_2, GATEWAY_DST["CNY"](318000), CSC(53000)));
        env(offer(MONEY_MAKER_2, CSC(209), MONEY_MAKER_2["CNY"](4.18)));
        env(offer(MONEY_MAKER_2, MONEY_MAKER_1["CNY"](990000), CSC(10000)));
        env(offer(MONEY_MAKER_2, MONEY_MAKER_1["CNY"](9990000), CSC(10000)));
        env(offer(MONEY_MAKER_2, GATEWAY_DST["CNY"](8870000), CSC(10000)));
        env(offer(MONEY_MAKER_2, CSC(232), MONEY_MAKER_2["CNY"](5.568)));
        env(offer(A2, CSC(2000), MONEY_MAKER_1["CNY"](66.8)));
        env(offer(A2, CSC(1200), GATEWAY_DST["CNY"](42)));
        env(offer(A2, MONEY_MAKER_1["CNY"](43.2), CSC(900)));
        env(offer(A3, MONEY_MAKER_1["CNY"](2240), CSC(50000)));
    ...
    void path_find_04()
    {
        ...
        env.fund(CSC(1000), G1BS, G2SW, A1, A2);
        env.fund(CSC(11000), M1);
        env.close();
    ...
    void path_find_05()
    {
        testcase("Path Find: non-CSC -> non-CSC, same currency");
        using namespace jtx;
        Env env(*this);
        Account A1 {"A1"};
        Account A2 {"A2"};
        ...
        env.fund(CSC(1000), A1, A2, A3, G1, G2, G3, G4);
        env.fund(CSC(10000), A4);
        env.fund(CSC(11000), M1, M2);
        env.close();
        ...
        env(offer(M2, CSC(10000), G2["HKD"](1000)));
        env(offer(M2, G1["HKD"](1000), CSC(10000)));
...

        {
            ...
            BEAST_EXPECT(same(st,
                ...
                stpath(M1),
                stpath(M2),
                stpath(IPE(cscIssue()), IPE(G2["HKD"]))
            ));
        }

        {
            ...
            BEAST_EXPECT(same(st,
                ...
                stpath(G1, IPE(cscIssue()), IPE(G2["HKD"]))
            ));
        }

        {
            //I4) CSC bridge" --
            //  Source -> AC -> OB to CSC -> OB from CSC -> AC -> Destination
            auto const& send_amt = A2["HKD"](10);
            std::tie(st, sa, da) = find_paths(env, A1, A2, send_amt,
                boost::none, G1["HKD"].currency);
            BEAST_EXPECT(equal(da, send_amt));
            ...
            BEAST_EXPECT(same(st,
                ...
                stpath(G1, IPE(cscIssue()), IPE(G2["HKD"]), G2)
            ));
        }
    }
    ...
    void path_find_06()
    {
        ...
        env.fund(CSC(11000), M1);
        env.fund(CSC(1000), A1, A2, A3, G1, G2);
        env.close();
