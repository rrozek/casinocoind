...
struct Regression_test : public beast::unit_test::suite
{
    ...
    void testOffer1()
    {
        ...
        env(offer("alice", USD(10), CSC(10)), require(owners("alice", 1)));
        env(offer("alice", USD(20), CSC(10)), json(R"raw(
                { "OfferSequence" : 2 }
    ...
    {
        testcase("Account balance < fee destroys correct amount of CSC");
        using namespace jtx;
        Env env(*this);
        env.memoize("alice");
            ...
            env.app().timeKeeper().closeTime());
        {
            ...
            auto const result = casinocoin::apply(env.app(),
        ...
        }
        {
            ...
            BEAST_EXPECT(balance == CSC(0));
        }
        expectedDrops -= aliceCSC * dropsPerCSC<int>::value;
        BEAST_EXPECT(next->info().drops == expectedDrops);
    }
