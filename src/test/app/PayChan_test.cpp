...
struct PayChan_test : public beast::unit_test::suite
{
    ...
    {
        Serializer msg;
        serializePayChanAuthorization (msg, channel, authAmt.csc ());
    ...
    testSimple ()
    {
        ...
        env (create (alice, bob, CSC (1000), settleDelay, pk));
        auto const chan = channel (*env.current (), alice, bob);
        BEAST_EXPECT (channelBalance (*env.current (), chan) == CSC (0));
        BEAST_EXPECT (channelAmount (*env.current (), chan) == CSC (1000));

        {
            auto const preAlice = env.balance (alice);
            env (fund (alice, chan, CSC (1000)));
            auto const feeDrops = env.current ()->fees ().base;
            BEAST_EXPECT (env.balance (alice) == preAlice - CSC (1000) - feeDrops);
        }

        auto chanBal = channelBalance (*env.current (), chan);
        auto chanAmt = channelAmount (*env.current (), chan);
        BEAST_EXPECT (chanBal == CSC (0));
        BEAST_EXPECT (chanAmt == CSC (2000));

        {
            // bad amounts (non-csc, negative amounts)
        ...
        // invalid account
        env (create (alice, "noAccount", CSC (1000), settleDelay, pk),
            ter (tecNO_DST));
        // can't create channel to the same account
        env (create (alice, alice, CSC (1000), settleDelay, pk),
             ter (temDST_IS_SRC));
        // invalid channel
        env (fund (alice, channel (*env.current (), alice, "noAccount"), CSC (1000)),
            ter (tecNO_ENTRY));
        // not enough funds
        env (create (alice, bob, CSC (10000), settleDelay, pk),
            ter (tecUNFUNDED));

        {
            // No signature claim with bad amounts (negative and non-csc)
            auto const iou = USDA (100).value ();
            auto const negCSC = CSC (-100).value ();
            auto const posCSC = CSC (100).value ();
            env (claim (alice, chan, iou, iou), ter (temBAD_AMOUNT));
            env (claim (alice, chan, posCSC, iou), ter (temBAD_AMOUNT));
            env (claim (alice, chan, iou, posCSC), ter (temBAD_AMOUNT));
            env (claim (alice, chan, negCSC, negCSC), ter (temBAD_AMOUNT));
            env (claim (alice, chan, posCSC, negCSC), ter (temBAD_AMOUNT));
            env (claim (alice, chan, negCSC, posCSC), ter (temBAD_AMOUNT));
        }
        {
            // No signature claim more than authorized
            auto const delta = CSC (500);
            auto const reqBal = chanBal + delta;
            auto const authAmt = reqBal + CSC (-100);
        ...
        }
        {
            ...
            auto const preBob = env.balance (bob);
            STAmount const authAmt = chanBal + CSC (500);
        ...
        env (fund (bob, chan, CSC (1000)), ter (tecNO_PERMISSION));
        BEAST_EXPECT (channelBalance (*env.current (), chan) == chanBal);
        BEAST_EXPECT (channelAmount (*env.current (), chan) == chanAmt);

        {
            // Wrong signing key
            auto const sig =
                signClaimAuth (bob.pk (), bob.sk (), chan, CSC (1500));
            env (claim (bob, chan, CSC (1500).value (), CSC (1500).value (),
                     Slice (sig), bob.pk ()),
                ter (temBAD_SIGNER));
            BEAST_EXPECT (channelBalance (*env.current (), chan) == chanBal);
            BEAST_EXPECT (channelAmount (*env.current (), chan) == chanAmt);
    ...
    testExpiration ()
    {
        ...
        env (fund (alice, chan, CSC (1),
            ...
            ter (temBAD_EXPIRATION));
        BEAST_EXPECT (!channelExpiration (*env.current (), chan));
        env (fund (alice, chan, CSC (1), NetClock::time_point{minExpiration}));
        env.close (minExpiration);
        // Try to extend the expiration after the expiration has already passed
        env (fund (
            alice, chan, CSC (1), NetClock::time_point{minExpiration + 1000s}));
        BEAST_EXPECT (!channelExists (*env.current (), chan));
    }
    ...
    testDepositAuth ()
    {
        ...
        auto USDA = alice["USD"];
        {
            ...
            env (claim (alice, chan,
                CSC (500).value(), CSC (500).value()), ter (tecNO_PERMISSION));
            env.close();
            ...
            auto const preBob = env.balance (bob);
            {
                auto const delta = CSC (500).value();
            ...
            }
            {
                ...
                auto const delta = CSC (600).value();
                auto const sig = signClaimAuth (pk, alice.sk (), chan, delta);
            ...
            }
            {
                ...
                auto const delta = CSC (800).value();
                ...
                env.close();
                BEAST_EXPECT (
                    env.balance (bob) == preBob + CSC (800) - (5 * baseFee));
            }
        }
    }
    ...
    testMalformedPK ()
    {
        ...
        auto jv = create (alice, bob, CSC (1000), settleDelay, pk);
...
BEAST_DEFINE_TESTSUITE (PayChan, app, casinocoin);
}  // test
}  // casinocoin
