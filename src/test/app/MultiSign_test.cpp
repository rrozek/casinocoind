...
public:
    void test_noReserve (FeatureBitset features)
    {
        ...
        auto const smallSignersReserve = reserve1 ? CSC(250) : CSC(350);
        ...
        }
        {
            ...
            // possible list.
            auto const addReserveBigSigners = reserve1 ? CSC(0) : CSC(350);
...
//      env(signers(alice, 1, { { bogie, 0x10000} }), ter (temBAD_WEIGHT));
    ...
    void test_masterSigners (FeatureBitset features)
    {
        ...
        env.fund(CSC(1000), alice, becky, cheri);
        env.close();
    ...
    void test_heterogeneousSigners (FeatureBitset features)
    {
        ...
        env.fund(CSC(1000), alice, becky, cheri, daria);
        env.close();
    ...
    void test_txTypes (FeatureBitset features)
    {
        ...
        env.fund(CSC(1000), alice, becky, zelda, gw);
        env.close();
        ...
        auto const baseFee = env.current()->fees().base;
        std::uint32_t aliceSeq = env.seq (alice);
        env(pay(alice, env.master, CSC(1)),
    ...
    void test_signForHash (FeatureBitset features)
    {
        ...
        // Use sign_for to sign a transaction where alice pays 10 CSC to
        // masterpassphrase.
