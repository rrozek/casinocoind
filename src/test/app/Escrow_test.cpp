...
struct Escrow_test : public beast::unit_test::suite
{
    ...
    testEnablement()
    {
        ...
        { // Escrow enabled
            Env env(*this);
            env.fund(CSC(5000), "alice", "bob");
            env(escrow("alice", "bob", CSC(1000)), finish_time(env.now() + 1s));
            env.close();

            auto const seq1 = env.seq("alice");

            env(escrow("alice", "bob", CSC(1000)), condition (cb1),
                finish_time(env.now() + 1s), fee(150000000));
            env.close();
            env(finish("bob", "alice", seq1),
                condition(cb1), fulfillment(fb1), fee(150000000));

            auto const seq2 = env.seq("alice");

            env(escrow("alice", "bob", CSC(1000)), condition(cb2),
                finish_time(env.now() + 1s), cancel_time(env.now() + 2s), fee(1500));
            env.close();
            env(cancel("bob", "alice", seq2), fee(150000000));
        }
    }
    ...
    testTiming()
    {
...

        {
            ...
            env(escrow("alice", "bob", CSC(1000)), finish_time(ts));

            // Advance the ledger, verifying that the finish won't complete
            // prematurely.
            for ( ; env.now() < ts; env.close())
                env(finish("bob", "alice", seq), fee(150000000), ter(tecNO_PERMISSION));

            env(finish("bob", "alice", seq), fee(150000000));
        }

        {
            ...
            auto const seq = env.seq("alice");
            env(escrow("alice", "bob", CSC(1000)), condition(cb1), cancel_time(ts));
            ...
            env(finish("bob", "alice", seq), condition(cb1),
                fulfillment(fb1), fee(150000000), ter(tecNO_PERMISSION));

            // Verify that the cancel will succeed
            env(cancel("bob", "alice", seq), fee(150000000));
        }

        {
            ...
            auto const seq = env.seq("alice");
            env(escrow("alice", "bob", CSC(1000)), finish_time(fts), cancel_time(cts));
    ...
    testTags()
    {
        ...
        env(escrow(alice, bob, CSC(1000)), finish_time(env.now() + 1s), ter(tecDST_TAG_NEEDED));

        // set source and dest tags
        auto const seq = env.seq(alice);

        env(escrow(alice, bob, CSC(1000)), finish_time(env.now() + 1s), stag(1), dtag(2));
    ...
    testDisallowCSC()
    {
        ...
        {
            // Respect the "asfDisallowCSC" account flag:
            Env env(*this, supported_amendments() - featureDepositAuth);

            env.fund(CSC(5000), "bob", "george");
            env(fset("george", asfDisallowCSC));
            env(escrow("bob", "george", CSC(10)), finish_time(env.now() + 1s),
                ter (tecNO_TARGET));
        }
        {
            // Ignore the "asfDisallowCSC" account flag, which we should
    ...
    test1571()
    {
...

        {
            ...
            env.fund(CSC(5000), "alice", "bob", "carol");
            env.close();

            // Creating an escrow without a finish time and finishing it
            // is allowed without fix1571:
            auto const seq1 = env.seq("alice");
            env(escrow("alice", "bob", CSC(100)), cancel_time(env.now() + 1s), fee(1500));
            env.close();
            env(finish("carol", "alice", seq1), fee(1500));
            BEAST_EXPECT (env.balance ("bob") == CSC(5100));
            ...
            auto const seq2 = env.seq("alice");
            env(escrow("alice", "bob", CSC(100)), cancel_time(env.now() + 1s),
                condition(cb1), fee(1500));
            env.close();
            env(finish("carol", "alice", seq2), condition(cb1),
                fulfillment(fb1), fee(1500));
            BEAST_EXPECT (env.balance ("bob") == CSC(5200));
        }

        {
            ...
            env (escrow("alice", "bob", CSC(100)), cancel_time(env.now() + 90s),
    ...
    testFails()
    {
        ...
        env(escrow("alice", "carol", CSC(1)), condition(cb1),
        ...
        { // Fail if the sender wants to send more than he has:
            ...
            env.fund (accountReserve + accountIncrement + CSC(50), "daniel");
            env(escrow("daniel", "bob", CSC(51)),
                finish_time(env.now() + 1s), ter(tecUNFUNDED));

            env.fund (accountReserve + accountIncrement + CSC(50), "evan");
            env(escrow("evan", "bob", CSC(50)),
                finish_time(env.now() + 1s), ter(tecUNFUNDED));

            env.fund (accountReserve, "frank");
            env(escrow("frank", "bob", CSC(1)),
                finish_time(env.now() + 1s), ter(tecINSUFFICIENT_RESERVE));
        }

        { // Specify incorrect sequence number
            env.fund (CSC(5000), "hannah");
            auto const seq = env.seq("hannah");
            env(escrow("hannah", "hannah", CSC(10)),
                finish_time(env.now() + 1s), fee(150000000));
            env.close();
            env(finish ("hannah", "hannah", seq + 7), fee(150000000), ter(tecNO_TARGET));
        }

        { // Try to specify a condition for a non-conditional payment
            env.fund (CSC(5000), "ivan");
            auto const seq = env.seq("ivan");

            env(escrow("ivan", "ivan", CSC(10)), finish_time(env.now() + 1s));
            env.close();
    ...
    testLockup()
    {
...

        {
            ...
            env.fund(CSC(5000), "alice", "bob");
            auto const seq = env.seq("alice");
            env(escrow("alice", "alice", CSC(1000)), finish_time(env.now() + 5s));
        ...
        }
        {
            ...
            Env env(*this);
            env.fund(CSC(5000), "alice", "bob", "zelda");
            auto const seq = env.seq("alice");
            env(escrow("alice", "bob", CSC(1000)), finish_time(env.now() + 5s));
            ...
            env.close();
            env.require(balance("alice", CSC(4000) - drops(10)));
            env.require(balance("bob", CSC(6000)));
            env.require(balance("zelda", CSC(5000) - drops(40)));
        }
        {
            ...
            env.require(balance("alice", CSC(4000) - (baseFee * 5)));
            env.require(balance("bob", CSC(6000) - (baseFee * 5)));
            env.require(balance("zelda", CSC(5000) - (baseFee * 4)));
        }
        {
            ...
            auto const baseFee = env.current()->fees().base;
            env.require(balance("alice", CSC(4000) - (baseFee * 2)));
            env.require(balance("bob", CSC(6000) - (baseFee * 2)));
            env.require(balance("zelda", CSC(5000) - (baseFee * 1)));
        }
        {
            // Conditional
            Env env(*this);
            env.fund(CSC(5000), "alice", "bob");
            auto const seq = env.seq("alice");
            env(escrow("alice", "alice", CSC(1000)), condition(cb2), finish_time(env.now() + 5s));
            env.require(balance("alice", CSC(4000) - drops(1000000)));
            ...
            env(finish("bob", "alice", seq),
                condition(cb2), fulfillment(fb2), fee(150000000));
        }
        {
            ...
            env.fund(CSC(5000), "alice", "bob");
            auto const seq = env.seq("alice");
            env(escrow("alice", "alice", CSC(1000)), condition(cb3), finish_time(env.now() + 5s));
            env.require(balance("alice", CSC(4000) - drops(1000000)));
            env.close();
            ...
            env(finish("alice", "alice", seq), condition(cb2),
                fulfillment(fb2), fee(150000000),                ter(tecCRYPTOCONDITION_ERROR));
            env(finish("bob", "alice", seq),   condition(cb3),
                fulfillment(fb3), fee(150000000),                ter(tecNO_PERMISSION));
            env(finish("alice", "alice", seq), condition(cb3),
                fulfillment(fb3), fee(150000000));
        }
        {
    ...
    testEscrowConditions()
    {
        ...
        { // Test cryptoconditions
            ...
            env(escrow("alice", "carol", CSC(1000)), condition(cb1),
            ...
            env(finish("bob", "alice", seq), condition(cb1),
                fulfillment(cb1), fee(150000000), ter(tecCRYPTOCONDITION_ERROR));
            BEAST_EXPECT((*env.le("alice"))[sfOwnerCount] == 1);
            env(finish("bob", "alice", seq), condition(cb1),
                fulfillment(cb2), fee(150000000), ter(tecCRYPTOCONDITION_ERROR));
            BEAST_EXPECT((*env.le("alice"))[sfOwnerCount] == 1);
            env(finish("bob", "alice", seq), condition(cb1),
                fulfillment(cb3), fee(150000000), ter(tecCRYPTOCONDITION_ERROR));
            BEAST_EXPECT((*env.le("alice"))[sfOwnerCount] == 1);
            ...
            env(finish("bob", "alice", seq), condition(cb2),
                fulfillment(fb1), fee(150000000), ter(tecCRYPTOCONDITION_ERROR));
            BEAST_EXPECT((*env.le("alice"))[sfOwnerCount] == 1);
            env(finish("bob", "alice", seq), condition(cb2),
                fulfillment(fb2), fee(150000000), ter(tecCRYPTOCONDITION_ERROR));
            BEAST_EXPECT((*env.le("alice"))[sfOwnerCount] == 1);
            env(finish("bob", "alice", seq), condition(cb2),
                fulfillment(fb3), fee(150000000), ter(tecCRYPTOCONDITION_ERROR));
            BEAST_EXPECT((*env.le("alice"))[sfOwnerCount] == 1);

            // Attempt to finish with the correct condition & fulfillment
            env(finish("bob", "alice", seq), condition(cb1),
                fulfillment(fb1), fee(150000000));
        ...
        { // Test cancel when condition is present
            ...
            auto const seq = env.seq("alice");
            BEAST_EXPECT((*env.le("alice"))[sfOwnerCount] == 0);
            env(escrow("alice", "carol", CSC(1000)), condition(cb2),
                cancel_time(env.now() + 1s));
            env.close();
            env.require(balance("alice", CSC(4000) - drops(1000000)));
...

        {
            ...
            env.close();
            auto const seq = env.seq("alice");
            env(escrow("alice", "carol", CSC(1000)), condition(cb3),
                cancel_time(env.now() + 1s));
            BEAST_EXPECT((*env.le("alice"))[sfOwnerCount] == 1);
        ...
        { // Test long & short conditions during creation
            ...
            env(escrow("alice", "carol", CSC(1000)), condition(Slice{p, s}),
                cancel_time(ts), ter(temMALFORMED));
            env(escrow("alice", "carol", CSC(1000)), condition(Slice{p, s - 1}),
                cancel_time(ts), ter(temMALFORMED));
            env(escrow("alice", "carol", CSC(1000)), condition(Slice{p, s - 2}),
                cancel_time(ts), ter(temMALFORMED));
            env(escrow("alice", "carol", CSC(1000)), condition(Slice{p + 1, s - 1}),
                cancel_time(ts), ter(temMALFORMED));
            env(escrow("alice", "carol", CSC(1000)), condition(Slice{p + 1, s - 3}),
                cancel_time(ts), ter(temMALFORMED));
            env(escrow("alice", "carol", CSC(1000)), condition(Slice{p + 2, s - 2}),
                cancel_time(ts), ter(temMALFORMED));
            env(escrow("alice", "carol", CSC(1000)), condition(Slice{p + 2, s - 3}),
                cancel_time(ts), ter(temMALFORMED));

            auto const seq = env.seq("alice");
            env(escrow("alice", "carol", CSC(1000)), condition(Slice{p + 1, s - 2}),
                cancel_time(ts), fee(10000000));
            env(finish("bob", "alice", seq),
                condition(cb1), fulfillment(fb1), fee(150000000));
        ...
        { // Test long and short conditions & fulfillments during finish
            ...
            // All these are expected to fail, because the
            // condition we pass in is malformed in some way
            env(escrow("alice", "carol", CSC(1000)), condition(Slice{cp, cs}),
                cancel_time(ts), ter(temMALFORMED));
            env(escrow("alice", "carol", CSC(1000)), condition(Slice{cp, cs - 1}),
                cancel_time(ts), ter(temMALFORMED));
            env(escrow("alice", "carol", CSC(1000)), condition(Slice{cp, cs - 2}),
                cancel_time(ts), ter(temMALFORMED));
            env(escrow("alice", "carol", CSC(1000)), condition(Slice{cp + 1, cs - 1}),
                cancel_time(ts), ter(temMALFORMED));
            env(escrow("alice", "carol", CSC(1000)), condition(Slice{cp + 1, cs - 3}),
                cancel_time(ts), ter(temMALFORMED));
            env(escrow("alice", "carol", CSC(1000)), condition(Slice{cp + 2, cs - 2}),
                cancel_time(ts), ter(temMALFORMED));
            env(escrow("alice", "carol", CSC(1000)), condition(Slice{cp + 2, cs - 3}),
                cancel_time(ts), ter(temMALFORMED));

            auto const seq = env.seq("alice");
            env(escrow("alice", "carol", CSC(1000)), condition(Slice{cp + 1, cs - 2}),
                cancel_time(ts), fee(10000000));
            ...
            env(finish("bob", "alice", seq), condition(Slice{cp + 1, cs - 2}),
                fulfillment(Slice{fp, fs}), fee(150000000), ter(tecCRYPTOCONDITION_ERROR));
            env(finish("bob", "alice", seq), condition(Slice{cp + 1, cs - 2}),
                fulfillment(Slice{fp, fs - 1}), fee(150000000), ter(tecCRYPTOCONDITION_ERROR));
            env(finish("bob", "alice", seq), condition(Slice{cp + 1, cs - 2}),
                fulfillment(Slice{fp, fs - 2}), fee(150000000), ter(tecCRYPTOCONDITION_ERROR));
            env(finish("bob", "alice", seq), condition(Slice{cp + 1, cs - 2}),
                fulfillment(Slice{fp + 1, fs - 1}), fee(150000000), ter(tecCRYPTOCONDITION_ERROR));
            env(finish("bob", "alice", seq), condition(Slice{cp + 1, cs - 2}),
                fulfillment(Slice{fp + 1, fs - 3}), fee(150000000), ter(tecCRYPTOCONDITION_ERROR));
            env(finish("bob", "alice", seq), condition(Slice{cp + 1, cs - 2}),
                fulfillment(Slice{fp + 1, fs - 3}), fee(150000000), ter(tecCRYPTOCONDITION_ERROR));
            env(finish("bob", "alice", seq), condition(Slice{cp + 1, cs - 2}),
                fulfillment(Slice{fp + 2, fs - 2}), fee(150000000), ter(tecCRYPTOCONDITION_ERROR));
            env(finish("bob", "alice", seq), condition(Slice{cp + 1, cs - 2}),
                fulfillment(Slice{fp + 2, fs - 3}), fee(150000000), ter(tecCRYPTOCONDITION_ERROR));

            // Now try for the right one
            env(finish("bob", "alice", seq), condition(cb2),
                fulfillment(fb2), fee(150000000));
            env.require(balance("alice", CSC(4000) - drops(10000000)));
            env.require(balance("carol", CSC(6000)));
        }
        { // Test empty condition during creation and
          // empty condition & fulfillment during finish
            ...
            env(escrow("alice", "carol", CSC(1000)), condition(Slice{}),
                cancel_time(env.now() + 1s), ter(temMALFORMED));
            ...
            env(finish("bob", "alice", seq), condition(Slice{}),
                fulfillment(Slice{}), fee(150000000), ter(tecCRYPTOCONDITION_ERROR));
            env(finish("bob", "alice", seq), condition(cb3),
                fulfillment(Slice{}), fee(150000000), ter(tecCRYPTOCONDITION_ERROR));
            env(finish("bob", "alice", seq), condition(Slice{}),
                fulfillment(fb3), fee(150000000), ter(tecCRYPTOCONDITION_ERROR));
        ...
        { // Test a condition other than PreimageSha256, which
          // would require a separate amendment
            ...
            env(escrow("alice", "bob", CSC(1000)), condition(cb),
                cancel_time(env.now() + 1s), ter(temMALFORMED));
        }
    }
    ...
    testMetaAndOwnership()
    {
...

        {
            ...
            env.fund(CSC(5000), alice, bruce, carol);
            auto const seq = env.seq(alice);
            env(escrow(alice, carol, CSC(1000)), finish_time(env.now() + 1s));
            ...
            casinocoin::Dir aod (*env.current(), keylet::ownerDir(alice.id()));
            BEAST_EXPECT(std::distance(aod.begin(), aod.end()) == 1);
            BEAST_EXPECT(std::find(aod.begin(), aod.end(), escrow) != aod.end());

            casinocoin::Dir cod (*env.current(), keylet::ownerDir(carol.id()));
...

        {
            ...
            env(escrow(alice, alice, CSC(1000)),
            ...
            env(escrow(bruce, bruce, CSC(1000)),
            ...
            {
                casinocoin::Dir bod(*env.current(), keylet::ownerDir(bruce.id()));
...

        {
            ...
            env(escrow(alice, bruce, CSC(1000)), finish_time(env.now() + 1s));
            BEAST_EXPECT((*env.meta())[sfTransactionResult] ==
                static_cast<std::uint8_t>(tesSUCCESS));
            env.close(5s);
            env(escrow(bruce, carol, CSC(1000)),
                finish_time(env.now() + 1s), cancel_time(env.now() + 2s));
            BEAST_EXPECT((*env.meta())[sfTransactionResult] ==
                static_cast<std::uint8_t>(tesSUCCESS));
            env.close(5s);
    ...
    void testConsequences()
    {
        ...
        {
            auto const jtx = env.jt(cancel("bob", "alice", 3), seq(1), fee(1000000));
            auto const pf = preflight(env.app(), env.current()->rules(),
                *jtx.stx, tapNONE, env.journal);
            BEAST_EXPECT(pf.ter == tesSUCCESS);
            auto const conseq = calculateConsequences(pf);
        ...
        {
            auto const jtx = env.jt(finish("bob", "alice", 3), seq(1), fee(1000000));
            auto const pf = preflight(env.app(), env.current()->rules(),
                *jtx.stx, tapNONE, env.journal);
            BEAST_EXPECT(pf.ter == tesSUCCESS);
            auto const conseq = calculateConsequences(pf);
