...
struct Escrow_test : public beast::unit_test::suite
{
    ...
    testEnablement()
    {
        ...
        { // Escrow not enabled
            ...
            env(lockup("alice", "bob", CSC(1000), env.now() + 1s), ter(temDISABLED));
        ...
        { // Escrow enabled
            Env env(*this);
            env.fund(CSC(5000), "alice", "bob");
            env(lockup("alice", "bob", CSC(1000), env.now() + 1s));
            env.close();
    ...
    testFails()
    {
        ...
        env (lockup("alice", "carol", CSC(0),
            env.now() + 1s),                            ter(temBAD_AMOUNT));
        env (lockup("alice", "carol", CSC(-1000),
            env.now() + 1s),                            ter(temBAD_AMOUNT));

        // Fail if neither CancelAfter nor FinishAfter are specified:
        {
            auto j1 = lockup("alice", "carol", CSC(1), env.now() + 1s);
            j1.removeMember ("FinishAfter");
            env (j1, ter(temBAD_EXPIRATION));

            auto j2 = condpay("alice", "carol", CSC(1), makeSlice(cb1), env.now() + 1s);
            j2.removeMember ("CancelAfter");
            env (j2, ter(temBAD_EXPIRATION));
        }

        // Fail if FinishAfter has already passed:
        env (lockup("alice", "carol", CSC(1), env.now() - 1s), ter (tecNO_PERMISSION));

        // Both CancelAfter and FinishAfter
        env(condpay("alice", "carol", CSC(1), makeSlice(cb1),
        ...
        { // Try to specify a condition for a non-conditional payment
            ...
            auto j = lockup("ivan", "ivan", CSC(10), env.now() + 1s);
    ...
    testLockup()
    {
        ...
        }
        {
            ...
            env.require(balance("alice", CSC(4000) - (baseFee * 3)));
            env.require(balance("bob", CSC(6000) - (baseFee * 3)));
    ...
    testEscrowConditions()
    {
        ...
        { // Test empty condition during creation and
          // empty condition & fulfillment during finish
            ...
            env(condpay("alice", "carol", CSC(1000), {}, T(S{1})), ter(temMALFORMED));
        ...
        { // Test a condition other than PreimageSha256, which
          // would require a separate amendment
            ...
            env(condpay("alice", "carol", CSC(1000), makeSlice(cb), T(S{1})),
                ter(temMALFORMED));
        }
    }
    ...
    testMetaAndOwnership()
    {
...

        {
            ...
            env(lockup(alice, carol, CSC(1000), env.now() + 1s));
...

        {
            ...
            env(lockup(alice, alice, CSC(1000), env.now() + 1s, env.now() + 500s));
            BEAST_EXPECT((*env.meta())[sfTransactionResult] == tesSUCCESS);
            env.close(5s);
            ...
            env(lockup(bruce, bruce, CSC(1000), env.now() + 1s, env.now() + 2s));
            BEAST_EXPECT((*env.meta())[sfTransactionResult] == tesSUCCESS);
            env.close(5s);
...

        {
            ...
            env(lockup(alice, bruce, CSC(1000), env.now() + 1s));
            BEAST_EXPECT((*env.meta())[sfTransactionResult] == tesSUCCESS);
            env.close(5s);
            env(lockup(bruce, carol, CSC(1000), env.now() + 1s, env.now() + 2s));
            BEAST_EXPECT((*env.meta())[sfTransactionResult] == tesSUCCESS);
            env.close(5s);
