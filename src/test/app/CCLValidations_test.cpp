...
public:
    ...
    void
    testLedgerTrieCCLValidatedLedger()
    {
        testcase("CCLValidatedLedger LedgerTrie");
        ...
        LedgerTrie<CCLValidatedLedger> trie;

        // First, create the single branch trie, with ledgers
        // separated by exactly 256 ledgers
        auto ledg_002 = CCLValidatedLedger{history[1], j};
        auto ledg_258 = CCLValidatedLedger{history[257], j};
        auto ledg_259 = CCLValidatedLedger{history[258], j};
        ...
        BEAST_EXPECT(
            trie.remove(CCLValidatedLedger{history[257], env.journal}, 1));
        trie.insert(CCLValidatedLedger{history[258], env.journal}, 1);
        trie.getPreferred(1);
        // trie.dump(std::cout);
        // 000000[0,1)(T:0,B:5)
