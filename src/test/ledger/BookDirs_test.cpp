...
struct BookDirs_test : public beast::unit_test::suite
{
    void test_bookdir(FeatureBitset features)
    {
        ...
        auto gw = Account("gw");
        auto USD = gw["USD"];
        env.fund(CSC(1000000), "alice", "bob", "gw");

        {
            Book book(cscIssue(), USD.issue());
            {
        ...
        {
            env(offer("alice", Account("alice")["USD"](50), CSC(10)));
            auto d = BookDirs(*env.current(),
                Book(Account("alice")["USD"].issue(), cscIssue()));
            BEAST_EXPECT(std::distance(d.begin(), d.end()) == 1);
        }

        {
            env(offer("alice", gw["CNY"](50), CSC(10)));
            auto d = BookDirs(*env.current(),
                Book(gw["CNY"].issue(), cscIssue()));
            BEAST_EXPECT(std::distance(d.begin(), d.end()) == 1);
        }
...

        {
            ...
            for (auto i = 1, j = 3; i <= 3; ++i, --j)
                for (auto k = 0; k < 80; ++k)
                    env(offer("alice", AUD(i), CSC(j)));

            auto d = BookDirs(*env.current(),
                Book(AUD.issue(), cscIssue()));
            BEAST_EXPECT(std::distance(d.begin(), d.end()) == 240);
            auto i = 1, j = 3, k = 0;
            for (auto const& e : d)
            {
                BEAST_EXPECT(e->getFieldAmount(sfTakerPays) == AUD(i));
                BEAST_EXPECT(e->getFieldAmount(sfTakerGets) == CSC(j));
                if (++k % 80 == 0)
                {
                    ++i;
                    --j;
