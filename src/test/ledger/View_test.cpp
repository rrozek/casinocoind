    ...
    : public beast::unit_test::suite
{
    ...
    testFlags()
    {
        ...
        env.trust(USD(100), alice, bob, carol);
        {
            ...
            env(offer(alice, CSC(5), USD(5)));

            // Now freeze gw.
            env(fset (gw, asfGlobalFreeze));
            env.close();
            env(offer(alice, CSC(4), USD(5)), ter(tecFROZEN));
            env.close();
        ...
        }
        {
            ...
            // carol's CSC balance should be her holdings minus her reserve.
            ...
            env(noop(carol), fee(carolsCSC + CSC(10)));
            env.close();

            // carol's CSC balance should now show as zero.
            BEAST_EXPECT(CSC(0) == accountHolds (*env.closed(),
                carol, cscCurrency(), gw, fhZERO_IF_FROZEN, env.journal));
        }
        {
            // accountFunds().
    ...
    testAreCompatible()
    {
        ...
        eA.fund(CSC(10000), alice);
        eA.close();
        auto const rdViewA3 = eA.closed();

        eA.fund(CSC(10000), bob);
        eA.close();
        ...
        eB.fund(CSC(10000), bob);
        eB.close();
        auto const rdViewB3 = eB.closed();

        eB.fund(CSC(10000), alice);
        eB.close();
    ...
    testRegressions()
    {
        ...
        // Make sure OpenLedger::empty works
        {
            ...
            env.fund(CSC(10000), Account("test"));
            BEAST_EXPECT(! env.app().openLedger().empty());
        }
    }
