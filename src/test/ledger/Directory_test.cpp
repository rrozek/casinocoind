...
struct Directory_test : public beast::unit_test::suite
{
    ...
    void testDirectoryOrdering()
    {
...

        {
            ...
            Env env(*this);
            env.fund(CSC(10000000), alice, gw);

            for (std::size_t i = 1; i <= 400; ++i)
                env(offer(alice, USD(i), CSC(i)));
            env.close();
            ...
            for (auto const& offer : book)
            {
                ...
                BEAST_EXPECT(offer->getFieldAmount(sfTakerGets) == CSC(count));
            }
        }
    }
    ...
    testRipd1353()
    {
        ...
        // should have no entries and be empty:
        {
            ...
            uint256 const bookBase = getBookBase({cscIssue(), USD.issue()});
...
BEAST_DEFINE_TESTSUITE_PRIO(Directory,ledger,casinocoin,1);
