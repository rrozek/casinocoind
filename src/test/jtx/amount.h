...
#ifndef CASINOCOIN_TEST_JTX_AMOUNT_H_INCLUDED
#define CASINOCOIN_TEST_JTX_AMOUNT_H_INCLUDED
...
The decision was made to accept amounts of drops and CSC
using an int type, since the range of CSC is 100 billion
...
/** Represents an CSC or IOU quantity
    ...
    CSC conversions from integer and floating point.
*/
struct PrettyAmount
{
private:
...
{
    AccountID account;
    casinocoin::Currency currency;

    BookSpec(AccountID const& account_,
        casinocoin::Currency const& currency_)
        : account(account_)
        , currency(currency_)
    {
    }
...
struct CSC_t
{
    ...
    */
    ...
    /** Returns an amount of CSC as STAmount

        @param v The number of CSC (not drops)
    */
    /** @{ */
    ...
    friend
    BookSpec
    operator~ (CSC_t const&)
    {        
        return BookSpec( cscAccount(),
            cscCurrency() );
    }
};

/** Converts to CSC Issue or STAmount.

    Examples:
        CSC         Converts to the CSC Issue
        CSC(10)     Returns STAmount of 10 CSC
*/
extern CSC_t const CSC;

/** Returns an CSC STAmount.
