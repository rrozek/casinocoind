...
#ifndef CASINOCOIN_TEST_JTX_ENV_H_INCLUDED
#define CASINOCOIN_TEST_JTX_ENV_H_INCLUDED
...
#include <casinocoin/ledger/CachedSLEs.h>
...
noCasinocoin (Account const& account, Args const&... args)
{
    return {{account, args...}};
}
...
public:
    ...
    */
    ...
    /** Returns the CSC balance on an account.
...
public:
    ...
    /** Create a new account with some CSC.
        ...
        Effects:
            The asfDefaultCasinocoin on the account is set,
            and the sequence number is incremented, unless
            the account is wrapped with a call to noCasinocoin.

            The account's CSC balance is set to amount.

            Generates a test that the balance is set.

        @param amount The amount of CSC to transfer to
                      each account.

        @param args A heterogeneous list of accounts to fund
                    or calls to noCasinocoin with lists of accounts
                    to fund.
    */
