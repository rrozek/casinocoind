...
/** Config: CASINOCOIN_VERIFY_NODEOBJECT_KEYS
    This verifies that the hash of node objects matches the payload.
    It is quite expensive so normally this is turned off!
*/
#ifndef   CASINOCOIN_VERIFY_NODEOBJECT_KEYS
//#define CASINOCOIN_VERIFY_NODEOBJECT_KEYS 1
#endif

/** Config: CASINOCOIN_DUMP_LEAKS_ON_EXIT
    Displays heap blocks and counted objects which were not disposed of
    during exit.
*/
#ifndef CASINOCOIN_DUMP_LEAKS_ON_EXIT
#define CASINOCOIN_DUMP_LEAKS_ON_EXIT 1
#endif

//------------------------------------------------------------------------------

// These control whether or not certain functionality gets
// compiled into the resulting casinocoind executable

/** Config: CASINOCOIN_ROCKSDB_AVAILABLE
    Controls whether or not the RocksDB database back-end is compiled into
    casinocoind. RocksDB requires a relatively modern C++ compiler (tested with
...
// is being written.
//
#ifndef CASINOCOIN_USE_VALIDATORS
#define CASINOCOIN_USE_VALIDATORS 0
#endif

/** Config: CASINOCOIN_SINGLE_IO_SERVICE_THREAD
    When set, restricts the number of threads calling io_service::run to one.
    This is useful when debugging.
*/
#ifndef CASINOCOIN_SINGLE_IO_SERVICE_THREAD
#define CASINOCOIN_SINGLE_IO_SERVICE_THREAD 0
#endif

// Uses OpenSSL instead of alternatives
#ifndef CASINOCOIN_USE_OPENSSL
#define CASINOCOIN_USE_OPENSSL 1
#endif
