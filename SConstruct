...
'''
    ...
    install         Default target and copies it to build/casinocoind (default)
...
GCC 5: If the gcc toolchain is used, gcc version 5 or better is required. On
    linux distros that ship with gcc 4 (ubuntu < 15.10), casinocoind will force gcc
    to use gcc4's ABI (there was an ABI change between versions). This allows us
    to use the package manager to install casinocoind dependencies. It also means if
...
Clang on linux: Clang cannot use the new gcc 5 ABI (clang does not know about
    ...
    link casinocoind in this scenario (use the `--static` with scons), as dynamic
...
sys.path.append(os.path.join('src', 'casinocoin', 'beast', 'site_scons'))
sys.path.append(os.path.join('src', 'casinocoin', 'site_scons'))
...
def check_openssl():
    ...
    if parse_time(d) < parse_time(build_time):
        raise Exception('Your openSSL was built on %s; '
                        'casinocoind needs a version built on or after %s.'
                        % (line, build_time))
...
base = Environment(
    toolpath=[os.path.join ('src', 'casinocoin', 'beast', 'site_scons', 'site_tools')],
...
def get_common_sources(toolchain):
    ...
    append_sources(
        result,
        'src/casinocoin/unity/secp256k1.cpp',
...
def get_classic_sources(toolchain):
    ...
    append_sources(
        result,
        *list_sources('src/casinocoin/core', '.cpp'),
        CPPPATH=[
            'src/soci/src/core',
            'src/sqlite']
    )
    append_sources(result, *list_sources('src/casinocoin/beast/clock', '.cpp'))
    append_sources(result, *list_sources('src/casinocoin/beast/container', '.cpp'))
    append_sources(result, *list_sources('src/casinocoin/beast/insight', '.cpp'))
    append_sources(result, *list_sources('src/casinocoin/beast/net', '.cpp'))
    append_sources(result, *list_sources('src/casinocoin/beast/utility', '.cpp'))
    append_sources(result, *list_sources('src/casinocoin/app', '.cpp'))
    append_sources(result, *list_sources('src/casinocoin/basics', '.cpp'))
    append_sources(result, *list_sources('src/casinocoin/conditions', '.cpp'))
    append_sources(result, *list_sources('src/casinocoin/crypto', '.cpp'))
    append_sources(result, *list_sources('src/casinocoin/consensus', '.cpp'))
    append_sources(result, *list_sources('src/casinocoin/json', '.cpp'))
    append_sources(result, *list_sources('src/casinocoin/ledger', '.cpp'))
    append_sources(result, *list_sources('src/casinocoin/legacy', '.cpp'))
    append_sources(result, *list_sources('src/casinocoin/net', '.cpp'))
    append_sources(result, *list_sources('src/casinocoin/overlay', '.cpp'))
    append_sources(result, *list_sources('src/casinocoin/peerfinder', '.cpp'))
    append_sources(result, *list_sources('src/casinocoin/protocol', '.cpp'))
    append_sources(result, *list_sources('src/casinocoin/rpc', '.cpp'))
    append_sources(result, *list_sources('src/casinocoin/shamap', '.cpp'))
    append_sources(result, *list_sources('src/casinocoin/server', '.cpp'))
    ...
    append_sources(
        result,
        *(list_sources('src/casinocoin/nodestore', '.cpp') + list_sources('src/test/nodestore', '.cpp')),
        CPPPATH=[
...
def get_unity_sources(toolchain):
    ...
    append_sources(
        ...
        'src/casinocoin/unity/basics.cpp',
    ...
    for toolchain in all_toolchains:
        for variant in variants:
            ...
            object_builder.add_source_files(
                ...
                'src/casinocoin/unity/beast.cpp',
                'src/casinocoin/unity/lz4.c',
                'src/casinocoin/unity/protobuf.cpp',
                'src/casinocoin/unity/casinocoin.proto.cpp',
            ...
            object_builder.add_source_files(
                'src/casinocoin/unity/ed25519_donna.c',
                CPPPATH=[
                    'src/ed25519-donna',
                ]
            )
            ...
            object_builder.add_source_files(
                'src/casinocoin/unity/snappy.cpp',
            ...
            if toolchain == "clang" and Beast.system.osx:
                object_builder.add_source_files('src/casinocoin/unity/beastobjc.mm')

            target = env.Program(
                target=os.path.join(variant_dir, 'casinocoind'),
...
# Build the list of casinocoind source files that hold unit tests
